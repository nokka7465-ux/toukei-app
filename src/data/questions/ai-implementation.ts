...
export const aiImplementationQuestions: Question[] = [
  ...
  { id: "aie-q66", category: "PyTorch", difficulty: 3, question: "**`torch.compile`**(PyTorch 2.0+)の用途として最も適切なものを選びなさい。", choices: ["モデルを JIT コンパイル → Inductor backend で高速化", "認証", "GPU 削減", "GUI"], correctIndex: 0, explanation: "**torch.compile**: PyTorch 2.x の目玉。**TorchDynamo + Inductor backend**(Triton kernel 生成)で 2x+ 高速化。**Eager Mode の自然さ + Graph Mode の高速性**を両立。" },
  { id: "aie-q67", category: "PyTorch", difficulty: 3, question: "**`torch.utils.data.IterableDataset`** の用途として最も適切なものを選びなさい。", choices: ["ストリーミングデータ(無限 / 巨大)で Iterator ベースに読込", "Map-style", "GUI", "認証"], correctIndex: 0, explanation: "**IterableDataset**: `__iter__` ベース ・ Sharding は自前(`worker_info`)・ 全量未知 / Streaming OK。**Map-style Dataset**(`__getitem__`)はインデックスアクセス前提。" },
  { id: "aie-q68", category: "Distributed", difficulty: 3, question: "**PyTorch FSDP(Fully Sharded Data Parallel)** の特徴として最も適切なものを選びなさい。", choices: ["重み + 勾配 + Optimizer State を全 GPU で分割 → 巨大モデル学習", "DDP と同じ", "GPU 削減", "認証"], correctIndex: 0, explanation: "**FSDP**: ZeRO Stage 3 相当。**重み / 勾配 / Optimizer State 全分割**。**DeepSpeed ZeRO** と並ぶ大規模学習標準。Llama / Mistral 学習で活用。" },
  { id: "aie-q69", category: "TensorFlow", difficulty: 3, question: "**`tf.function`** の役割として最も適切なものを選びなさい。", choices: ["Python 関数を Graph にコンパイル → 高速化 + 移植性", "Eager Mode", "GUI", "認証"], correctIndex: 0, explanation: "**`@tf.function`**: AutoGraph で Python → Graph(Concrete Function)。**Tracing**(初回)で Graph 生成 → 以降高速実行。**input_signature**で再 Trace 防止。" },
  { id: "aie-q70", category: "JAX", difficulty: 3, question: "**JAX** の特徴として **誤っているもの** を選びなさい。", choices: ["`jit / grad / vmap / pmap` 等の関数変換", "XLA バックエンドで TPU / GPU / CPU 統一", "Pure Function + Functional Style", "Eager のみ"], correctIndex: 3, explanation: "**JAX**: **NumPy 互換 + Autograd + XLA**。**Pure Function + Functional**(副作用なし)。**Flax / Haiku / Equinox / Optax** がエコシステム。Google Research の主力 FW。" },
  { id: "aie-q71", category: "Hugging Face", difficulty: 3, question: "**Hugging Face Datasets** ライブラリの利点として最も適切なものを選びなさい。", choices: ["Apache Arrow ベース + メモリマップ + Lazy + Stream + 5,000+ データセット Hub", "GPU 学習", "認証", "Cost"], correctIndex: 0, explanation: "**HF Datasets**: Arrow / mmap → 巨大データもメモリ効率良。**Stream Mode**(無限 / 巨大データ)・ **map / filter / push_to_hub**。**transformers / accelerate / peft / trl** と統合。" },
  { id: "aie-q72", category: "PEFT", difficulty: 3, question: "**PEFT(Parameter-Efficient Fine-Tuning)** の手法として **誤っているもの** を選びなさい。", choices: ["LoRA / QLoRA", "Prefix / Prompt Tuning", "Adapter / IA³", "Full Fine-tune"], correctIndex: 3, explanation: "**PEFT**: 小パラメータのみ学習。**LoRA**(低ランク行列)・ **QLoRA**(4bit base + LoRA)・ **Prefix / Prompt Tuning**(埋込追加)・ **Adapter**(層挿入)・ **IA³**(スケーリング)。Full FT は対極。" },
  { id: "aie-q73", category: "vLLM", difficulty: 3, question: "**vLLM** の特徴として最も適切なものを選びなさい。", choices: ["PagedAttention で KV Cache 効率 + 高速 LLM 推論", "学習用", "GUI", "認証"], correctIndex: 0, explanation: "**vLLM**(UC Berkeley): **PagedAttention**(KV Cache をページング ・ メモリ断片化解消)・ **Continuous Batching** → スループット 5-20x。**Production LLM Serving** で標準化中。" },
  { id: "aie-q74", category: "Serving", difficulty: 3, question: "**Triton Inference Server**(NVIDIA)の特徴として最も適切なものを選びなさい。", choices: ["複数 FW(TF / PyTorch / ONNX / TensorRT)を統一サーブ + Dynamic Batching", "TF 専用", "GUI", "認証"], correctIndex: 0, explanation: "**Triton**: マルチ FW(TF / PT / ONNX / TensorRT / Python BLS / vLLM)・ **Dynamic / Adaptive Batching**・ Model Ensemble ・ A/B Routing ・ Metrics。GPU / CPU / Edge 対応。" },
  { id: "aie-q75", category: "ONNX", difficulty: 3, question: "**ONNX Runtime** の最適化として **誤っているもの** を選びなさい。", choices: ["Graph Optimization(Constant Folding / Op Fusion)", "Execution Provider(CUDA / TensorRT / OpenVINO / DirectML)", "INT8 / FP16 量子化", "Code Refactoring"], correctIndex: 3, explanation: "**ONNX Runtime**: Graph 最適化 + EP(CUDA / TRT / OpenVINO / CoreML / DirectML / ROCm)+ 量子化。**Cross-Platform 推論の事実標準**。" },
  { id: "aie-q76", category: "Quantization", difficulty: 3, question: "**bitsandbytes** で **4-bit / 8-bit Quantization** を Hugging Face と組合せる利点として最も適切なものを選びなさい。", choices: ["巨大 LLM(70B+)を 1 GPU で動作可能に", "GPU 不要", "GUI", "認証"], correctIndex: 0, explanation: "**bitsandbytes**: 4bit(NF4)/ 8bit Quantization + **LLM.int8()** 行列乗算。**QLoRA**(4bit base + LoRA)で 70B 級も 24GB GPU で学習可能に。" },
  { id: "aie-q77", category: "MLflow", difficulty: 3, question: "**MLflow Tracking** の記録対象として **誤っているもの** を選びなさい。", choices: ["Params / Metrics / Artifacts", "Model / Source / Tags", "Run ID / Experiment ID", "GPU シリアル番号"], correctIndex: 3, explanation: "**MLflow Tracking**: `log_param / log_metric / log_artifact / log_model`。**UI / REST / SDK** で記録。**Model Registry**(本番昇格)・ **Deployment**(Bedrock / SageMaker / Azure ML)。" },
  { id: "aie-q78", category: "DVC", difficulty: 3, question: "**DVC(Data Version Control)** の用途として最も適切なものを選びなさい。", choices: ["大規模データ / モデルのバージョン管理 + Git 連携", "コード VCS", "認証", "GPU"], correctIndex: 0, explanation: "**DVC**: Git ライク UI でデータ / モデルをハッシュ管理 → S3 / GCS / Azure Blob 等に Push。**Reproducibility**(コード + データ + 環境)で MLOps 必須ツール。" },
  { id: "aie-q79", category: "Observability", difficulty: 3, question: "**LLM Observability** ツールとして **誤っているもの** を選びなさい。", choices: ["Langfuse / Helicone / LangSmith", "Arize Phoenix / Weights & Biases", "PromptLayer / Traceloop", "Microsoft Word"], correctIndex: 3, explanation: "**LLM Obs**: Trace / Cost / Latency / Quality 監視。**Langfuse**(OSS)・ **LangSmith**(LangChain)・ **Helicone / Arize / W&B Weave / Phoenix / Traceloop / PromptLayer**。" },
  { id: "aie-q80", category: "Cert", difficulty: 2, question: "**AI 実装検定 S 級** の特徴として最も適切なものを選びなさい。", choices: ["最上位 ・ PyTorch / TF / Hugging Face + MLOps 実装力を総合判定", "ITパスポート相当", "GUI 操作のみ", "数学のみ"], correctIndex: 0, explanation: "**AI 実装検定**: B(基礎)→ A(中級)→ **S(上級)**。**S 級**: 実装 + 学習ループ + 評価 + Serving + 量子化 ・ 蒸留 + Drift 検知 + 倫理。**E 資格(JDLA)との橋渡し**。" },
];
