...
export const gTestQuestions: Question[] = [
  ...
  { id: "gt-q75", difficulty: 3, category: "LLM", question: "**Mixture of Experts(MoE)** アーキテクチャの利点として最も適切なものを選びなさい。", choices: ["大規模パラメータでも推論時はサブセット(Active)のみ使用 → 計算効率", "全パラメータ常時使用", "GPU 不要", "学習データ不要"], correctIndex: 0, explanation: "**MoE**: Router が各 Token を Expert 群の一部に振り分け → **総パラメータ大 ・ Active パラメータ小**。**Mixtral / DBRX / GPT-4 / DeepSeek-V3** が代表。スケーリング則の効率化。" },
  { id: "gt-q76", difficulty: 3, category: "LLM", question: "**RLHF(Reinforcement Learning from Human Feedback)** の 3 ステップとして最も適切なものを選びなさい。", choices: ["SFT → Reward Model 学習 → PPO で LLM 最適化", "Pretrain のみ", "GAN 学習", "K-Means"], correctIndex: 0, explanation: "**RLHF**: ① Supervised Fine-Tuning ② 人間嗜好データで Reward Model 学習 ③ **PPO** で LLM を Reward 最大化に調整。ChatGPT / Claude / Gemini の差別化要素。**DPO**(Direct Preference Optimization)で簡素化版も。" },
  { id: "gt-q77", difficulty: 3, category: "Prompting", question: "**Self-Consistency** プロンプト技法の特徴として最も適切なものを選びなさい。", choices: ["複数回 CoT 推論 → 多数決で最頻回答を採用 → 精度向上", "1 回のみ実行", "確率最大選択", "GPU 削減"], correctIndex: 0, explanation: "**Self-Consistency**(Wang et al. 2022): CoT を Temperature > 0 で複数生成 → Majority Voting。**数学 / 論理問題で大幅精度向上**。**Tree of Thoughts / Graph of Thoughts** にも発展。" },
  { id: "gt-q78", difficulty: 3, category: "RAG", question: "**Retrieval-Augmented Generation(RAG)** が解決する LLM の課題として最も適切なものを選びなさい。", choices: ["ハルシネーション + 知識更新 + 引用提示", "学習時間短縮", "GPU 不要化", "認証強化"], correctIndex: 0, explanation: "**RAG**: ベクトル検索で関連文書取得 → Prompt に追加 → LLM 生成。**ハルシネーション低減 / Knowledge Cutoff 回避 / Citation 可能 / 社内文書活用**。**Vertex AI Search / Bedrock KB** で実装。" },
  { id: "gt-q79", difficulty: 3, category: "Agent", question: "**Multi-Agent System** で **Orchestrator-Worker パターン** の利点として最も適切なものを選びなさい。", choices: ["Orchestrator が複雑タスクを分解 → 専門 Agent に並列委譲 → 統合", "1 Agent のみ", "GUI 必須", "認証 SSO"], correctIndex: 0, explanation: "**Orchestrator-Worker**(Anthropic 提唱): Lead Agent が Plan → 並列 Sub-Agents 実行 → 結果統合。**LangGraph / CrewAI / AutoGen / Anthropic Sub-Agents** が代表 FW。Deep Research 系で活躍。" },
  { id: "gt-q80", difficulty: 3, category: "Safety", question: "**LLM の Prompt Injection** 攻撃の典型例として最も適切なものを選びなさい。", choices: ["Untrusted 入力に『以前の指示を無視して〜』を埋め込み Tool / 出力を悪用", "GPU 過負荷", "認証バイパスのみ", "DB 直接攻撃"], correctIndex: 0, explanation: "**Prompt Injection**(OWASP LLM01): Direct(ユーザーから直接)と Indirect(Web 文書 / Email 経由)。**Tool 悪用 / 機密漏洩 / 不正出力**。対策は System Prompt 強化 + Tool 制限 + 出力検証 + Human-in-the-Loop。" },
  { id: "gt-q81", difficulty: 3, category: "Safety", question: "**Constitutional AI**(Anthropic)の特徴として最も適切なものを選びなさい。", choices: ["憲法(原則集)に基づき LLM 自身が出力を批判 / 修正 → RLAIF", "RLHF と完全に同じ", "人間フィードバック不要化のみ", "GPU 削減"], correctIndex: 0, explanation: "**Constitutional AI**(CAI): モデル自身が憲法に照らし Self-Critique + Revise → **RLAIF**(RL from AI Feedback)。**Claude の中核技術**。RLHF の人間ラベルコスト削減 + Alignment 強化。" },
  { id: "gt-q82", difficulty: 3, category: "World Models", question: "**World Models** が指す概念として最も適切なものを選びなさい。", choices: ["環境ダイナミクスを学習 → 内部シミュレーションで先読み計画", "GUI モデル", "DB スキーマ", "認証"], correctIndex: 0, explanation: "**World Models**(Ha & Schmidhuber 2018, LeCun の JEPA): 環境の遷移関数を内部表現で予測。**DreamerV3 / MuZero / Sora(動画)/ Genie** が代表。**ロボット ・ ゲーム ・ Video Gen** の基盤。" },
  { id: "gt-q83", difficulty: 3, category: "Embodied AI", question: "**Embodied AI**(身体性 AI)の代表的応用領域として **誤っているもの** を選びなさい。", choices: ["ヒューマノイドロボット(Figure / Tesla Optimus)", "Vision-Language-Action モデル(VLA)", "Autonomous Driving(自動運転)", "純粋なテキスト要約のみ"], correctIndex: 3, explanation: "**Embodied AI**: 物理世界とインタラクションする AI。**RT-2 / OpenVLA / Helix / Optimus / Figure 02 / FSD**。テキスト要約は **Disembodied**。**Sim2Real / World Models** が主要技術。" },
  { id: "gt-q84", difficulty: 3, category: "倫理", question: "**AI 著作権** の現状(2024-2025 時点)として **誤っているもの** を選びなさい。", choices: ["学習データの著作権利用は各国で議論中(米国は Fair Use 巡る訴訟多数)", "日本は著作権法 30 条の 4 で AI 学習に寛容", "生成物の著作権帰属は法域で異なる", "全世界で AI 著作権完全統一"], correctIndex: 3, explanation: "**AI 著作権**: 米国(Fair Use 巡る判例形成中、NYT v OpenAI 等)・ 日本(30 条の 4 で学習寛容)・ EU(AI Act + 著作権指令)。**全世界統一はまだ達成されていない**。" },
  { id: "gt-q85", difficulty: 3, category: "EU AI Act", question: "**EU AI Act** の **リスク分類** で High Risk に含まれるものとして **誤っているもの** を選びなさい。", choices: ["医療診断補助", "信用スコアリング", "履歴書スクリーニング", "ゲーム NPC"], correctIndex: 3, explanation: "**EU AI Act リスク階層**: ① Unacceptable(社会信用 ・ サブリミナル等、禁止)② High(医療 / 信用 / 採用 / 法執行など)③ Limited(チャットボット、開示義務)④ Minimal(ゲーム NPC 等、規制なし)。" },
  { id: "gt-q86", difficulty: 3, category: "AI Safety", question: "**Mechanistic Interpretability** が目指すものとして最も適切なものを選びなさい。", choices: ["NN 内部の特徴量 ・ 回路を解析し挙動を機械的に説明", "GUI 改善", "学習速度向上", "Endpoint 高速化"], correctIndex: 0, explanation: "**Mech Interp**(Anthropic / OpenAI 研究): Sparse Autoencoder で Feature 抽出 → Circuit 解析。**Alignment 確立 / Jailbreak 防止 / Deception 検知** に必須。Claude / GPT 系で研究進展。" },
  { id: "gt-q87", difficulty: 3, category: "Scaling Laws", question: "**Chinchilla Scaling Law**(DeepMind 2022)が示した教訓として最も適切なものを選びなさい。", choices: ["パラメータ数とデータ量は同等にスケール(Compute 最適な比率)", "パラメータ最優先", "データ最優先", "GPU 数のみ"], correctIndex: 0, explanation: "**Chinchilla**: GPT-3(175B / 300B Token)は **データ不足**。**最適配分**: 計算予算 C ≈ 6ND(N=params, D=tokens)・ N:D ≈ 1:20。Llama 3 / Gemini など以降の学習設計に影響。" },
  { id: "gt-q88", difficulty: 3, category: "Multimodal", question: "**Omni-Modal Model**(GPT-4o / Gemini 1.5 / Claude 3.5 Sonnet)の特徴として最も適切なものを選びなさい。", choices: ["Text / 画像 / 音声 / 動画を単一モデルで統合処理", "テキスト専用", "音声専用", "画像専用"], correctIndex: 0, explanation: "**Omni-Modal**(2024+): GPT-4o(リアルタイム音声 ・ 画像 ・ ビデオ)・ Gemini 1.5/2.0 / Claude 3.5/4 / Llama 3.2 Vision / Pixtral。Tokenizer も Multimodal(画像 / 音声 → トークン化)。" },
  { id: "gt-q89", difficulty: 3, category: "Reasoning", question: "**OpenAI o1 / o3 / Claude Extended Thinking** など **Reasoning Model** の特徴として最も適切なものを選びなさい。", choices: ["推論時に Chain-of-Thought を内部で延長 → 数学 / コードで大幅精度向上", "学習速度向上のみ", "GPU 削減", "認証強化"], correctIndex: 0, explanation: "**Reasoning Models**(2024 末〜): **Test-Time Compute Scaling** で推論時に思考時間を延長。**o1 / o3 / DeepSeek-R1 / Claude 3.7+ / Gemini 2.0 Thinking** が代表。**RLHF + Self-Play** で訓練。AIME / MATH / GPQA で人間超え。" },
];
