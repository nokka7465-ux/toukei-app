...
export const awsAiPractitionerQuestions: Question[] = [
  ...
  { id: "awsai-q66", category: "Bedrock", difficulty: 3, question: "**Amazon Bedrock Knowledge Bases** の用途として最も適切なものを選びなさい。", choices: ["マネージド RAG: S3 / Web 等から取込 → ベクトル DB → Citation 付き応答", "新規 LLM 学習", "GPU 提供", "認証 SSO"], correctIndex: 0, explanation: "**Bedrock KB**: マネージド RAG(取込 → Embedding → OpenSearch/Aurora 等 → Retrieve → Generate)。**Hierarchical Chunking / Metadata Filter / GraphRAG / Reranking**(2024+)対応。" },
  { id: "awsai-q67", category: "Bedrock", difficulty: 3, question: "**Bedrock Agents** の主要構成要素として **誤っているもの** を選びなさい。", choices: ["Action Group(API 呼出)", "Knowledge Base(RAG)", "Guardrails(Safety)", "GPU 直接アクセス"], correctIndex: 3, explanation: "**Bedrock Agents 構成**: Foundation Model + Action Group(Lambda/API)+ Knowledge Base + Guardrails + Memory + Trace。**Multi-Agent Collaboration**(2024 末)で Lead + Sub Agents も。" },
  { id: "awsai-q68", category: "Bedrock", difficulty: 3, question: "**Bedrock Guardrails** が検出 / ブロックする項目として **誤っているもの** を選びなさい。", choices: ["Denied Topics(禁止トピック)", "PII / 機密データの露出", "Contextual Grounding(ハルシネーション)", "GPU 利用率"], correctIndex: 3, explanation: "**Guardrails**: Denied Topics / Content Filter(暴力 ・ 性 ・ 憎悪)/ PII Detection / Word Filter / **Contextual Grounding Check**(RAG での乖離検出)。複数モデル間で共有可。" },
  { id: "awsai-q69", category: "Bedrock", difficulty: 3, question: "**Bedrock の Inference 形態** として **誤っているもの** を選びなさい。", choices: ["On-Demand(従量)", "Provisioned Throughput(予約)", "Cross-Region Inference(2024+)", "GPU 直購入"], correctIndex: 3, explanation: "**Bedrock Inference**: On-Demand / Provisioned Throughput / Batch / **Cross-Region Inference**(複数リージョン分散で容量確保 + 低レイテンシ)/ **Latency-Optimized**(2024+)。" },
  { id: "awsai-q70", category: "SageMaker", difficulty: 3, question: "**SageMaker Canvas** の用途として最も適切なものを選びなさい。", choices: ["No-Code / Low-Code で ML モデル作成(ビジネスアナリスト向け)", "学習用 GPU", "認証", "Cost 管理"], correctIndex: 0, explanation: "**SageMaker Canvas**: No-Code ML + 自然言語(Q)。**Tabular Prediction / Time Series / 画像分類 / 文章分類**。**Foundation Model 接続**(Bedrock / Cohere / AI21)も可能。" },
  { id: "awsai-q71", category: "SageMaker", difficulty: 3, question: "**SageMaker Clarify** の主要機能として **誤っているもの** を選びなさい。", choices: ["Bias 検出(Pre / Post-training)", "SHAP-based Explainability", "ML Drift / Fairness Monitoring", "GPU 監視"], correctIndex: 3, explanation: "**SageMaker Clarify**: Pre-training Bias(DPL / KL 等)・ Post-training Bias(DPPL / DI 等)・ **SHAP Explainability** ・ **Model Monitor 統合で Drift / Fairness 継続監視**。Responsible AI の中核。" },
  { id: "awsai-q72", category: "Pricing", difficulty: 3, question: "**Bedrock 料金体系** の主要項目として **誤っているもの** を選びなさい。", choices: ["Input Token 単価", "Output Token 単価", "Cached Input(Prompt Caching)", "GPU 時間課金"], correctIndex: 3, explanation: "**Bedrock 料金**: Input / Output Token + **Prompt Caching**(再利用入力に割引、2024 末-)+ Batch(50% off)+ Cross-Region Inference + Provisioned Throughput(時間)。GPU 時間ではなく Token ベース。" },
  { id: "awsai-q73", category: "Security", difficulty: 3, question: "**Bedrock のデータ取扱い** に関する事実として **誤っているもの** を選びなさい。", choices: ["顧客データは Foundation Model の再学習に使用されない", "VPC Endpoint 経由でアクセス可", "Customer Managed KMS で暗号化可", "全顧客データを Anthropic に転送"], correctIndex: 3, explanation: "**Bedrock データ取扱い**: 顧客データはモデル提供者(Anthropic / Cohere / AI21 等)に共有されない・ Provider 学習にも使用されない・ AWS で隔離。**VPC Endpoint + KMS + Service Control Policies** で統制。" },
  { id: "awsai-q74", category: "Agents", difficulty: 3, question: "**Multi-Agent Collaboration**(Bedrock Agents)の構造として最も適切なものを選びなさい。", choices: ["Supervisor Agent が Sub-agents を Orchestration", "全 Agent が並列独立", "Agent 1 つのみ", "GPU 共有"], correctIndex: 0, explanation: "**Multi-Agent Collaboration**(2024 GA): Supervisor + 専門 Sub-Agents の階層構造。**複雑タスクを Sub に委譲 → 統合**。LangGraph / CrewAI 等の OSS パターンに対応。" },
  { id: "awsai-q75", category: "Cost 戦略", difficulty: 3, question: "**Generative AI のコスト最適化** で **Model Distillation**(Bedrock 2024)の用途として最も適切なものを選びなさい。", choices: ["大規模モデルの出力で小規模モデルを学習 → コスト ・ レイテンシ削減", "GPU 増設", "認証", "DB クエリ"], correctIndex: 0, explanation: "**Model Distillation**(Bedrock 2024 GA): Teacher Model(Claude Sonnet 等)出力で Student(Haiku 等)を学習 → **特定タスクで精度維持 + 大幅安価 / 低レイテンシ**。" },
  { id: "awsai-q76", category: "Architecture", difficulty: 3, question: "**RAG on AWS** の **基本アーキテクチャ** として最も適切なものを選びなさい。", choices: ["S3 → Bedrock KB / OpenSearch Vector → Bedrock LLM → Citation", "EC2 のみ", "Lambda のみ", "RDS のみ"], correctIndex: 0, explanation: "**RAG on AWS**: ① S3(文書)→ ② Embedding(Titan / Cohere)→ ③ **OpenSearch Vector / Aurora pgvector / Bedrock KB** ④ Retrieve → ⑤ Bedrock LLM 生成 + Citation。**API Gateway + Lambda + Streamlit** で UI。" },
  { id: "awsai-q77", category: "倫理", difficulty: 3, question: "**AWS Responsible AI の柱** として **誤っているもの** を選びなさい。", choices: ["Controllability / Privacy / Safety / Fairness", "Transparency / Explainability / Governance / Veracity", "Robustness", "GPU 性能"], correctIndex: 3, explanation: "**AWS Responsible AI**(2024 発表): Controllability / Privacy & Security / Safety / Fairness / Veracity & Robustness / Explainability / Transparency / Governance。**8 つの Dimensions**。GPU 性能は範囲外。" },
  { id: "awsai-q78", category: "業界", difficulty: 3, question: "**AWS 生成 AI ユースケース** で **コンタクトセンター強化** に直結するサービスとして最も適切なものを選びなさい。", choices: ["Amazon Connect + Bedrock + Transcribe + Polly + Lex", "EC2 のみ", "S3 のみ", "VPC のみ"], correctIndex: 0, explanation: "**コンタクトセンター AI**: Connect(IVR / Routing)+ **Transcribe**(STT)+ Bedrock(回答生成 / 要約)+ **Polly**(TTS)+ **Lex**(NLU / Bot)+ Contact Lens(分析)。" },
  { id: "awsai-q79", category: "認定", difficulty: 2, question: "**AWS AI Practitioner(AIF-C01)** の **位置づけ** として最も適切なものを選びなさい。", choices: ["Foundational レベル ・ Cloud Practitioner(CCP)と同等難易度の AI 入門", "Professional レベル", "Speciality レベル", "Associate レベル"], correctIndex: 0, explanation: "**AIF-C01**(2024 リリース): **Foundational**(CCP と並ぶ最易レベル)・ ビジネス向け生成 AI / ML 概観 + Responsible AI + 業務適用。**MLA-C01(Associate)/ MLS-C01(Specialty)** が上位。" },
  { id: "awsai-q80", category: "実務", difficulty: 3, question: "AWS AI Practitioner で重視される **総合判断力** として最も適切なものを選びなさい。", choices: ["生成 AI / 古典 ML / Bedrock / SageMaker / Responsible AI / コスト / セキュリティの統合判断", "ML 実装のみ", "GUI 操作のみ", "数学のみ"], correctIndex: 0, explanation: "**AIF-C01 試験範囲**: ① AI / ML / Generative AI Fundamentals ② Bedrock / SageMaker / Q ファミリ ③ Prompting / RAG / Fine-tuning ④ Responsible AI / Safety / Bias ⑤ Security / Privacy ⑥ Cost / 最適化。**ビジネス + 技術 + ガバナンス**の総合。" },
];
