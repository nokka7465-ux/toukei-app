...
export const gradeThreeQuestions: Question[] = [
  ...
  { id: "g3-q66", category: "代表値", difficulty: 2, question: "**外れ値を含むデータ** の代表値として最も適切なものを選びなさい。", choices: ["中央値(外れ値に頑健)", "平均(外れ値で偏る)", "最大値", "範囲"], correctIndex: 0, explanation: "**外れ値耐性**: 中央値 > 平均。**Robust 統計**(中央値 / 四分位 / IQR / MAD)。**Trimmed Mean / Winsorized Mean** で外れ値の影響を軽減する方法もある。" },
  { id: "g3-q67", category: "散らばり", difficulty: 2, question: "**標準偏差** と **分散** の関係として正しいものを選びなさい。", choices: ["標準偏差 = √分散", "標準偏差 = 分散²", "標準偏差 = 分散 + 1", "両者同じ"], correctIndex: 0, explanation: "**$\\sigma = \\sqrt{\\sigma^2}$**。**分散は単位が二乗**(cm² 等)、**標準偏差は元の単位**で解釈しやすい。**不偏分散**(n-1 で割る)・ **標本分散**(n で割る)の区別も重要。" },
  { id: "g3-q68", category: "確率分布", difficulty: 2, question: "**正規分布 N(0,1)** で **±1σ 範囲に含まれる確率** として正しいものを選びなさい。", choices: ["約 68%", "約 50%", "約 95%", "約 99%"], correctIndex: 0, explanation: "**68-95-99.7 ルール**: ±1σ ≒ 68% ・ ±2σ ≒ 95% ・ ±3σ ≒ 99.7%。**標準正規表 / Z 値**で任意の区間確率を求められる。" },
  { id: "g3-q69", category: "確率分布", difficulty: 2, question: "**標準化(z 変換)** の式として正しいものを選びなさい。", choices: ["$z = (x - \\mu) / \\sigma$", "$z = x \\cdot \\sigma + \\mu$", "$z = x / \\mu$", "$z = x - \\sigma$"], correctIndex: 0, explanation: "**標準化**: 任意の正規 → 標準正規 N(0,1)。**偏差値 = 50 + 10z**(平均 50 ・ 標準偏差 10 にスケール)。**Z スコア**で異常検知 ・ 比較も。" },
  { id: "g3-q70", category: "二項分布", difficulty: 2, question: "**二項分布 B(n, p)** の期待値と分散として正しいものを選びなさい。", choices: ["E = np、Var = np(1-p)", "E = p、Var = np", "E = np²、Var = p", "両者 n"], correctIndex: 0, explanation: "**Binomial B(n, p)**: $E[X] = np$、$Var[X] = np(1-p)$。**n 大かつ p 中庸** で正規近似 N(np, np(1-p))。**ベルヌーイ試行の和**として導出。" },
  { id: "g3-q71", category: "推定", difficulty: 2, question: "**点推定** と **区間推定** の違いとして最も適切なものを選びなさい。", choices: ["点=1 つの値、区間=範囲 + 信頼度(95% CI 等)", "両者同じ", "点=範囲", "区間=固定値"], correctIndex: 0, explanation: "**点推定**: 標本平均で母平均推定。**区間推定**: 95% CI のような範囲 + 信頼水準。**広いほど高確度だが情報量低**。標本サイズ ↑ で CI 狭く。" },
  { id: "g3-q72", category: "推定", difficulty: 2, question: "**95% 信頼区間** の意味として最も適切なものを選びなさい。", choices: ["同じ手順で多くの標本から CI を作ると、約 95% が真値を含む", "母平均が CI 内にある確率 95%", "標本平均が CI 内", "誤差ゼロ"], correctIndex: 0, explanation: "**頻度主義 CI**: 手順を繰返すと 95% が真値を含む。**ベイズ信用区間(Credible Interval)** は事後分布の 95% 範囲(直感的解釈)で別概念。" },
  { id: "g3-q73", category: "仮説検定", difficulty: 2, question: "**p 値 0.03** の解釈として最も適切なものを選びなさい。", choices: ["帰無仮説の下で観測以上の結果が出る確率が 3% → α=5% で棄却", "母平均が 3%", "誤差 3%", "効果量 3%"], correctIndex: 0, explanation: "**p 値**: H₀ 下で観測以上の極端結果が出る確率。**< α(0.05 等)で H₀ 棄却**。**「H₀ が真である確率」ではない**(典型誤解)。効果量と併せて報告が現代標準。" },
  { id: "g3-q74", category: "仮説検定", difficulty: 2, question: "**第 1 種の過誤(α)** と **第 2 種の過誤(β)** の関係として最も適切なものを選びなさい。", choices: ["α 減らすと β 増 ・ サンプル増で両方減", "α=β", "両者独立", "α 増で β 減"], correctIndex: 0, explanation: "**α / β トレードオフ**: 厳しくすると Type II 増。**サンプル増**で両方同時に減らせる。**検出力 = 1 - β**(MDE 検出能力)。実験計画の基本。" },
  { id: "g3-q75", category: "相関", difficulty: 2, question: "**ピアソン相関係数 $r$** の取りうる値の範囲として正しいものを選びなさい。", choices: ["-1 ≤ r ≤ 1", "0 ≤ r ≤ 1", "0 ≤ r ≤ 100", "r ≥ 0"], correctIndex: 0, explanation: "**ピアソン $r$**: -1(完全負)〜 0(無相関)〜 +1(完全正)。**線形関係のみ**(非線形は捉えない)。**スピアマン / ケンドールの順位相関** も。**r² が決定係数 R²**。" },
  { id: "g3-q76", category: "回帰", difficulty: 2, question: "**単回帰** $y = a + bx$ の **$b$**(傾き)の意味として最も適切なものを選びなさい。", choices: ["x が 1 増えると y が平均 b 増", "y の切片", "残差", "誤差分散"], correctIndex: 0, explanation: "**傾き b**: x 1 単位変化での y の平均変化量。**切片 a**: x=0 のときの y の予測値。**最小二乗法**で a, b を推定。" },
  { id: "g3-q77", category: "クロス集計", difficulty: 2, question: "**$\\chi^2$ 適合度検定** の用途として最も適切なものを選びなさい。", choices: ["観測度数が特定の分布(理論度数)に従うかを検定", "平均比較", "回帰係数検定", "GPU 検定"], correctIndex: 0, explanation: "**$\\chi^2 = \\sum (O - E)^2 / E$**。**適合度検定**(分布形)・ **独立性検定**(クロス表)・ **均一性検定**。期待度数 5 以上が条件。" },
  { id: "g3-q78", category: "サンプリング", difficulty: 2, question: "**無作為抽出** が大事な理由として最も適切なものを選びなさい。", choices: ["標本に偏りなく母集団を代表 ・ 推定の不偏性確保", "サンプル数増", "GPU 速度", "コスト最小"], correctIndex: 0, explanation: "**無作為抽出**: 全員に等しい確率で抽選 → 標本の代表性 ・ 不偏推定の前提。**層化 / 系統 / 多段** など実用的方法も。**便宜抽出は偏り**で結果不適。" },
  { id: "g3-q79", category: "可視化", difficulty: 2, question: "**箱ひげ図(Box Plot)** で読み取れない情報として最も適切なものを選びなさい。", choices: ["分布の細かい形状(山が 1 つか 2 つか等)", "中央値 / 四分位", "外れ値", "範囲"], correctIndex: 0, explanation: "**Box Plot**: 5 数要約 + 外れ値。**多峰性 ・ 細かい形は不可視**。**Violin Plot**(箱ひげ+密度推定)で形状も把握。" },
  { id: "g3-q80", category: "実務", difficulty: 2, question: "統計検定 3 級で重視される **データ分析リテラシ** として最も適切なものを選びなさい。", choices: ["記述統計 + 確率分布 + 推定 / 検定の基礎 + 実データ解釈の総合", "数学公式のみ", "GUI 操作のみ", "Excel のみ"], correctIndex: 0, explanation: "**3 級試験範囲**: ① 記述統計(代表値 ・ 散らばり ・ グラフ)② 確率分布(二項 ・ 正規)③ 推定 ・ 検定の基礎 ④ 相関 ・ 単回帰 ⑤ 実データ解釈。**大学初年度〜社会人入門**レベル。" },
  { id: "g3-q81", category: "代表値", difficulty: 2, question: "**平均が中央値より大きい場合** に分布が示す形として最も適切なものを選びなさい。", choices: ["右に裾が長い(右に歪んだ分布 ・ Right-skewed)", "左に裾が長い", "対称", "ベル型"], correctIndex: 0, explanation: "**平均 > 中央値 → 右に長い裾(年収 / 売上 / 株価)**。**平均 < 中央値 → 左に長い裾**。**両者一致 → 対称分布(正規等)**。歪度(Skewness)で定量化。" },
  { id: "g3-q82", category: "散らばり", difficulty: 2, question: "**変動係数(CV: Coefficient of Variation)** の定義として正しいものを選びなさい。", choices: ["標準偏差 / 平均(× 100%)", "分散 × 平均", "中央値 / 範囲", "平均 / 標準偏差"], correctIndex: 0, explanation: "**CV = σ / μ**(無次元)。**異なる単位 / スケールのばらつき比較**に有効(身長と体重の散らばり比較 等)。**μ ≒ 0 では不安定**になるので使用注意。" },
  { id: "g3-q83", category: "確率", difficulty: 2, question: "**独立な 2 事象 A, B** での **$P(A \\cup B)$** の式として正しいものを選びなさい。", choices: ["$P(A) + P(B) - P(A)P(B)$", "$P(A) P(B)$", "$P(A) + P(B)$", "$0$"], correctIndex: 0, explanation: "**加法定理**: $P(A \\cup B) = P(A) + P(B) - P(A \\cap B)$。**独立なら** $P(A \\cap B) = P(A) P(B)$ → 上の式。**排反なら** $P(A \\cap B) = 0$ → $P(A) + P(B)$。" },
  { id: "g3-q84", category: "確率分布", difficulty: 2, question: "**正規分布で偏差値 70 の上位順位** として最も近いものを選びなさい。", choices: ["上位 2.3%(±2σ)", "上位 16%", "上位 50%", "上位 0.1%"], correctIndex: 0, explanation: "**偏差値 70 = +2σ → 上位 2.3%**(片側)。**60 = +1σ → 16%**、**80 = +3σ → 0.13%**。**68-95-99.7 ルール**から導けます。" },
  { id: "g3-q85", category: "確率分布", difficulty: 2, question: "**t 分布** の形状として最も適切なものを選びなさい。", choices: ["正規分布より裾が厚く ・ 自由度大で正規に収束", "正規より尖る", "一様", "離散"], correctIndex: 0, explanation: "**t 分布**: 標本サイズ小で **裾が厚い**(外れ値の影響受けやすい)。**自由度 ≧ 30 で正規にほぼ一致**。**$\\sigma$ 未知の小標本**の母平均 CI / 検定で使用。" },
  { id: "g3-q86", category: "推定", difficulty: 2, question: "**標本平均の標準誤差(SE)** の式として正しいものを選びなさい。", choices: ["$SE = \\sigma / \\sqrt{n}$(母 σ)、$s / \\sqrt{n}$(標本 s)", "$\\sigma$", "$n$", "$\\sigma \\cdot n$"], correctIndex: 0, explanation: "**SE**: 標本平均の散らばり。**$n$ で割るので n 増で SE 減**(誤差半減には n を 4 倍)。**CI**: $\\bar{x} \\pm 1.96 \\cdot SE$(95%、$\\sigma$ 既知時)。" },
  { id: "g3-q87", category: "検定", difficulty: 2, question: "**両側検定 vs 片側検定** の使い分けとして最も適切なものを選びなさい。", choices: ["方向を問わない=両側、特定方向のみ気にする=片側", "両者同じ", "両側=厳しい", "片側=保守的"], correctIndex: 0, explanation: "**両側**: $H_1: \\mu \\neq \\mu_0$(方向問わず)。**片側**: $H_1: \\mu > \\mu_0$ または $\\mu < \\mu_0$。**片側の方が検出力高**(同 n / 同 α)だが事前の方向想定が必要。" },
  { id: "g3-q88", category: "回帰", difficulty: 2, question: "**回帰直線 $y = 3 + 2x$** で $x = 10$ のときの予測値を選びなさい。", choices: ["23", "20", "13", "30"], correctIndex: 0, explanation: "$y = 3 + 2 \\cdot 10 = 23$。**切片(3)+ 傾き(2)× x** の直線。**外挿(範囲外 x)は危険**(モデル仮定が崩れる可能性)。" },
  { id: "g3-q89", category: "可視化", difficulty: 2, question: "**ヒストグラムの階級幅(bin width)** が **狭すぎる** と起こる典型問題を選びなさい。", choices: ["ノイズが目立ち真の分布が見えにくい", "情報損失", "解像度低", "GPU"], correctIndex: 0, explanation: "**Bin が狭い → ノイズ目立つ / 広い → 平滑化されすぎ → 情報損失**。**Sturges / Scott / Freedman-Diaconis** の経験則で適切な幅を計算可能。" },
  { id: "g3-q90", category: "実務", difficulty: 2, question: "**生存時間データ**(イベントまでの時間)を扱う統計手法の **入り口** として最も適切なものを選びなさい。", choices: ["Kaplan-Meier 推定量(打ち切り対応)", "t 検定", "OLS 回帰", "ヒストグラム"], correctIndex: 0, explanation: "**生存分析**(Survival Analysis): 打ち切りデータ(Censoring)を扱う特殊な統計。**Kaplan-Meier**(非パラ)・ **Cox 比例ハザード**(共変量込み)。**医療 ・ 保険 ・ 信用リスク**で必須。3 級では概念のみ。" },
  { id: "g3-q91", category: "分散", difficulty: 2, question: "**標本分散 $s^2$** と **不偏分散 $u^2$** の関係として正しいものを選びなさい。", choices: ["$u^2 = \\frac{n}{n-1} s^2$", "$u^2 = s^2$", "$u^2 = \\frac{n-1}{n} s^2$", "無関係"], correctIndex: 0, explanation: "**標本分散** $s^2 = \\frac{1}{n}\\sum (x_i - \\bar{x})^2$、**不偏分散** $u^2 = \\frac{1}{n-1}\\sum (x_i - \\bar{x})^2$ なので $u^2 = \\frac{n}{n-1} s^2$。**ベッセル補正** で過小評価を矯正。" },
  { id: "g3-q92", category: "推定", difficulty: 2, question: "**母平均 $\\mu$ の 95% 信頼区間** の **幅** を **狭く** する方法として最も適切なものを選びなさい。", choices: ["標本サイズ $n$ を増やす", "信頼度を上げる", "標準偏差を増やす", "片側にする"], correctIndex: 0, explanation: "幅は $z \\cdot \\sigma/\\sqrt{n}$ に比例。**$n$ を 4 倍にすると幅は半分**。信頼度を上げる(99%)と幅は広がる方向。" },
  { id: "g3-q93", category: "検定", difficulty: 2, question: "**第 1 種誤り** の定義として正しいものを選びなさい。", choices: ["帰無仮説が真なのに棄却する誤り (有意水準 α)", "帰無仮説が偽なのに採択する誤り", "対立仮説の誤り", "計算ミス"], correctIndex: 0, explanation: "**第 1 種誤り (Type I)**: H0 真なのに棄却 (= 偽陽性)。確率 α。**第 2 種誤り (Type II)**: H0 偽なのに採択 (= 偽陰性)。確率 β。**検出力 = 1 - β**。" },
  { id: "g3-q94", category: "二項分布", difficulty: 2, question: "**二項分布 $B(n, p)$** の **期待値** として正しいものを選びなさい。", choices: ["$np$", "$np(1-p)$", "$\\sqrt{np}$", "$n+p$"], correctIndex: 0, explanation: "**$E[X] = np$**、**$V[X] = np(1-p)$**。コイン 100 回 (n=100, p=0.5) なら期待値 50 回、分散 25、標準偏差 5。" },
  { id: "g3-q95", category: "ポアソン", difficulty: 2, question: "**ポアソン分布** の特徴として **誤っているもの** を選びなさい。", choices: ["期待値と分散が異なる", "稀な事象の出現回数を表す", "$P(X=k) = \\frac{\\lambda^k e^{-\\lambda}}{k!}$", "$\\lambda$ が大きいと正規分布で近似"], correctIndex: 0, explanation: "**ポアソン分布の特徴**: 期待値 = 分散 = $\\lambda$ (同じ)。**二項分布で $n \\to \\infty, p \\to 0, np = \\lambda$ 一定** のとき導出される極限分布。電話発生 / 故障件数 / 来店数。" },
  { id: "g3-q96", category: "標本分布", difficulty: 2, question: "**中心極限定理** の主張として正しいものを選びなさい。", choices: ["標本平均の分布は $n$ が大きいと正規分布に近づく", "母集団が正規分布", "全データが正規", "中央値が正規分布"], correctIndex: 0, explanation: "**CLT**: 元の母集団がどんな分布でも、$n \\geq 30$ 程度で標本平均 $\\bar{X}$ の分布は近似的に $N(\\mu, \\sigma^2/n)$。**統計的推測の理論的基盤**。" },
  { id: "g3-q97", category: "相関", difficulty: 2, question: "**ピアソン相関係数 $r = 0.85$** の解釈として最も適切なものを選びなさい。", choices: ["強い正の線形相関がある", "因果関係がある", "85% の精度で予測可能", "完全な相関"], correctIndex: 0, explanation: "$r \\geq 0.7$ で強い正相関、$0.3 \\leq r < 0.7$ で中、$|r| < 0.3$ で弱い。**相関 ≠ 因果**。$r^2$ が決定係数 (説明率)。" },
  { id: "g3-q98", category: "回帰", difficulty: 2, question: "**最小二乗法**(OLS)が最小化する量として正しいものを選びなさい。", choices: ["残差平方和 $\\sum (y_i - \\hat{y}_i)^2$", "残差の絶対和", "予測値の和", "係数の和"], correctIndex: 0, explanation: "**OLS**: $\\min \\sum (y - \\hat{y})^2$。微分してゼロで正規方程式。**外れ値に弱い** → LAD (絶対残差) や **ロバスト回帰** で対応可。" },
  { id: "g3-q99", category: "確率", difficulty: 2, question: "**ベイズの定理** $P(A|B) = \\frac{P(B|A) P(A)}{P(B)}$ の **応用** として正しいものを選びなさい。", choices: ["医療検査の陽性適中率算定", "回帰係数推定", "最尤推定", "頻度分布"], correctIndex: 0, explanation: "**ベイズ定理**: 事前確率を観測で更新。**医療検査の有名な逆問題** (病気の人が陽性 vs 陽性者の中で病気の人の割合は別物)。スパムフィルタ / 故障診断 / A/B テストにも応用。" },
  { id: "g3-q100", category: "実務", difficulty: 2, question: "**統計検定 3 級合格後** の次のステップとして最も推奨されるものを選びなさい。", choices: ["統計検定 2 級 (推測統計の本格学習)", "1 級に直接挑戦", "再受験", "別資格"], correctIndex: 0, explanation: "**3 級 → 2 級** が標準ルート。2 級は推測統計の核心 (推定 / 検定 / 回帰)。**実務適性なら DS 基礎 → DS 発展** の並行受験も有効。" },
];
