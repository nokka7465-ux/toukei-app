...
export const dsExpertQuestions: Question[] = [
  ...
  { id: "dsx-q51", category: "Bayesian", difficulty: 3, question: "**MCMC の収束診断** で **R-hat(Gelman-Rubin)統計量** の判定基準として最も適切なものを選びなさい。", choices: ["R-hat < 1.01 で収束(複数チェーン間 / 内分散の比較)", "R-hat > 2 で収束", "R-hat = 0 で完璧", "判定不能"], correctIndex: 0, explanation: "**R-hat**: チェーン間分散 vs チェーン内分散の比。**R-hat → 1 で収束**。慣習的に < 1.01 が許容。**ESS(Effective Sample Size)** も併用。" },
  { id: "dsx-q52", category: "因果推論", difficulty: 3, question: "**因果媒介分析(Mediation Analysis)** の **間接効果** の意味として最も適切なものを選びなさい。", choices: ["Mediator(媒介)を経由する効果", "全効果", "残差効果", "GPU 効果"], correctIndex: 0, explanation: "**Mediation**: $T \\to M \\to Y$ の経路。**Direct Effect**(M を介さず)+ **Indirect Effect**(M 経由)= **Total Effect**。Baron-Kenny の手法 ・ Causal Mediation Analysis(Pearl)で。" },
  { id: "dsx-q53", category: "深層学習", difficulty: 3, question: "**Transformer の Self-Attention** の **計算量** として正しいものを選びなさい。", choices: ["O(n² d)(系列長 n、埋込次元 d)", "O(n d)", "O(d²)", "O(log n)"], correctIndex: 0, explanation: "**Self-Attention**: Query × Key^T = $O(n^2 d)$。**長系列で 2 乗的に増加** → **Linformer / Performer / FlashAttention / Mamba(SSM)** で線形化試行。" },
  { id: "dsx-q54", category: "Generative", difficulty: 3, question: "**Diffusion モデル** の **学習目標** として最も適切なものを選びなさい。", choices: ["各 timestep でノイズを予測(L2 / MSE Loss)", "確率密度を最大化", "Cross-Entropy", "MSE のみ"], correctIndex: 0, explanation: "**DDPM**: $L = E_{t, x_0, \\epsilon} [\\| \\epsilon - \\epsilon_\\theta(x_t, t) \\|^2]$。**$t$ のタイムステップで加えたノイズを予測**。Score Matching とも等価。" },
  { id: "dsx-q55", category: "Optimization", difficulty: 3, question: "**SGD の確率近似** の収束条件として正しいものを選びなさい。", choices: ["学習率 $\\eta_t$ が $\\sum \\eta_t = \\infty, \\sum \\eta_t^2 < \\infty$(Robbins-Monro)", "$\\eta$ 固定", "$\\eta = 1$", "GPU 必須"], correctIndex: 0, explanation: "**Robbins-Monro 条件**: 学習率総和は発散(十分に動く)+ 二乗和は収束(分散制御)。例: $\\eta_t = 1/t$ は満たす、定数は満たさない(理論)。" },
  { id: "dsx-q56", category: "情報理論", difficulty: 3, question: "**相互情報量 $I(X; Y)$** の意味として正しいものを選びなさい。", choices: ["X と Y が共有する情報量(Y を知ることで X の不確実性が減る量)", "X の分散", "Y の標準偏差", "Cross-Entropy"], correctIndex: 0, explanation: "**Mutual Information**: $I(X;Y) = H(X) - H(X|Y) = D_{KL}(P(X,Y) \\| P(X)P(Y))$。**特徴量選択 ・ 表現学習 ・ Variational Autoencoder** で活用。" },
  { id: "dsx-q57", category: "強化学習", difficulty: 3, question: "**Bellman 方程式** の役割として最も適切なものを選びなさい。", choices: ["価値関数の漸化式 $V(s) = E[r + \\gamma V(s')]$", "勾配計算", "BatchNorm", "K-Means"], correctIndex: 0, explanation: "**Bellman 方程式**: 価値関数の自己参照。**Bellman 期待 / 最適方程式 → Value Iteration / Policy Iteration / Q-Learning / TD Learning** の理論基盤。" },
  { id: "dsx-q58", category: "強化学習", difficulty: 3, question: "**TD(Temporal Difference)学習** の主な利点として最も適切なものを選びなさい。", choices: ["MC 法と違いエピソード終了を待たず逐次更新可能", "完全モデル必要", "オフライン専用", "GPU 必須"], correctIndex: 0, explanation: "**TD**: ブートストラップ(現推定で次推定を更新)。**Q-Learning / SARSA / Actor-Critic / DQN** の基礎。**MC 法はエピソード完了まで待つ**(高分散)。" },
  { id: "dsx-q59", category: "Generative", difficulty: 3, question: "**VAE と GAN** の主な違いとして最も適切なものを選びなさい。", choices: ["VAE: 明示的事後 + 再構成、GAN: 暗黙生成 + 敵対的", "両者同じ", "VAE は廃止", "GAN は再構成"], correctIndex: 0, explanation: "**VAE**: 潜在変数の事後分布を変分近似 + ELBO 最大化 → ぼやけがち。**GAN**: G vs D の敵対学習 → 鮮明だが Mode Collapse。**Diffusion** が両者の利点を統合的に解決。" },
  { id: "dsx-q60", category: "高次元", difficulty: 3, question: "**Curse of Dimensionality(次元の呪い)** の代表的問題として **誤っているもの** を選びなさい。", choices: ["距離計算の意味希薄化", "サンプル密度の指数的低下", "過学習リスク増", "GPU 利用率向上"], correctIndex: 3, explanation: "**Curse of Dimensionality**: 高次元で **距離 ・ 確率 ・ 学習** が困難に。**次元削減**(PCA / Manifold Learning / Autoencoder)・ **特徴量選択** で対処。GPU は無関係。" },
  { id: "dsx-q61", category: "Online Learning", difficulty: 3, question: "**Online Learning** の **Regret** の定義として最も適切なものを選びなさい。", choices: ["事後最適と比較した累積損失差(最小化目標)", "学習速度", "GPU 削減", "Calibration"], correctIndex: 0, explanation: "**Regret**: $R_T = \\sum_{t=1}^{T} L_t(\\hat{\\theta}_t) - \\min_\\theta \\sum L_t(\\theta)$。**Sublinear Regret**(o(T))で『学習している』と判定。Online Gradient Descent / Multi-Armed Bandit。" },
  { id: "dsx-q62", category: "Causal", difficulty: 3, question: "**RDD(Regression Discontinuity Design)** の用途として最も適切なものを選びなさい。", choices: ["閾値付近で処置の有無が変わる時の因果効果推定", "Random Sampling", "MCMC", "GPU 監視"], correctIndex: 0, explanation: "**RDD**: 閾値で処置の有無が断絶 → 閾値付近を Quasi-RCT として扱う。**学校入試合格点 / 補助金カットオフ** などの自然実験で活用。" },
  { id: "dsx-q63", category: "実務", difficulty: 3, question: "**DS チームの組織形態** で **Center of Excellence(CoE)モデル** の特徴として最も適切なものを選びなさい。", choices: ["DS 部門を中央集約 + 各事業部に伴走支援", "完全分散", "Outsource", "GPU 共有のみ"], correctIndex: 0, explanation: "**CoE**: 中央 DS 部門が事業部支援(Hub-and-Spoke)。**完全埋込 / Federated / Outsource** などのモデルと比較で **教育 ・ 標準化 ・ 知識共有** が強み。" },
  { id: "dsx-q64", category: "プロダクション ML", difficulty: 3, question: "**ML パイプラインの再現性** を保つ要素として **誤っているもの** を選びなさい。", choices: ["コード(Git)/ データ(DVC / LakeFS)/ 環境(Docker / Conda)/ ハイパーパラメータ", "MLflow / Weights & Biases で記録", "Pipeline 化(Kubeflow / Airflow)", "実行ごとにランダムシード変更"], correctIndex: 3, explanation: "**再現性**: Git + DVC + Docker + Hyperparam 記録 + **固定 Random Seed**。実行ごとシード変更は **逆**(再現不能)。" },
  { id: "dsx-q65", category: "AI Safety", difficulty: 3, question: "**AI Alignment** の主目的として最も適切なものを選びなさい。", choices: ["AI システムの行動を人間の意図 ・ 価値観に整合させる", "GPU 数増加", "DB 暗号化", "認証強化"], correctIndex: 0, explanation: "**Alignment**: RLHF / DPO / Constitutional AI / Reward Modeling などで AI の振る舞いを人間の意図に整合。**AI Safety**(Anthropic / OpenAI / DeepMind 重点)の中核研究領域。" },
  { id: "dsx-q66", category: "因果推論", difficulty: 4, question: "**Double Machine Learning(DML)** の特徴として最も適切なものを選びなさい。", choices: ["Nuisance 関数を ML で推定 + Neyman 直交化で半パラ効率推定", "全 ML 学習", "GUI 推定", "認証"], correctIndex: 0, explanation: "**DML / Debiased ML**(Chernozhukov et al. 2018): 高次元共変量を ML(GBDT / NN)で扱いつつ因果効果を $\\sqrt{n}$ 一致 + 漸近正規。**EconML / DoubleML** で実装。" },
  { id: "dsx-q67", category: "因果推論", difficulty: 4, question: "**回帰不連続デザイン(RDD)** の用途として最も適切なものを選びなさい。", choices: ["閾値前後で処置が不連続に変化 → 閾値近傍で因果効果を識別", "RCT 代替全般", "GPU", "認証"], correctIndex: 0, explanation: "**RDD**(Thistlethwaite & Campbell 1960): 試験合否 / 補助金閾値 / 投票比率 50% など。**Sharp**(完全不連続)/ **Fuzzy**(確率的)。**Local Polynomial / Bandwidth Selection** が肝。" },
  { id: "dsx-q68", category: "ベイズ", difficulty: 4, question: "**Stan / PyMC / Pyro** に代表される **確率的プログラミング** の利点として最も適切なものを選びなさい。", choices: ["モデルを宣言的に書くだけで NUTS / HMC / VI が自動", "学習データ生成", "GUI 設計", "GPU 不要"], correctIndex: 0, explanation: "**PPL**: モデル記述 + 自動推論。**Stan / PyMC**(NUTS)・ **Pyro**(SVI + NumPyro JAX)・ **TensorFlow Probability / Edward2**。**Hierarchical Bayesian / Multilevel Model** が容易に。" },
  { id: "dsx-q69", category: "Bayesian Deep", difficulty: 4, question: "**Bayesian Neural Network** の主アプローチとして **誤っているもの** を選びなさい。", choices: ["Variational Inference(BBB / MFVI)", "MC Dropout(Gal & Ghahramani 2016)", "Laplace Approximation / SWAG", "Bubble Sort"], correctIndex: 3, explanation: "**Bayesian NN**: 重みを確率分布として扱い不確実性推定。**VI / MC Dropout / Laplace / SWAG / Deep Ensembles / SGLD**。**Aleatoric vs Epistemic Uncertainty** の分離が重要。" },
  { id: "dsx-q70", category: "強化学習", difficulty: 4, question: "**PPO(Proximal Policy Optimization)** の特徴として最も適切なものを選びなさい。", choices: ["Clipped Surrogate Objective で更新を制限 → 安定 + サンプル効率", "Q-Learning と同じ", "GPU 不要", "認証"], correctIndex: 0, explanation: "**PPO**(Schulman et al. 2017): $L^{CLIP} = E[\\min(r_t A_t, clip(r_t, 1-\\epsilon, 1+\\epsilon) A_t)]$。**TRPO 簡素化**。**RLHF / 連続制御 / ロボット**で標準。" },
  { id: "dsx-q71", category: "強化学習", difficulty: 4, question: "**Model-Based RL** の利点として最も適切なものを選びなさい。", choices: ["World Model 学習でサンプル効率向上 + 計画(MPC / MCTS)可能", "Model-Free と同等", "GUI", "認証"], correctIndex: 0, explanation: "**Model-Based**: Dyna / MuZero / DreamerV3 / PlaNet。**Model-Free**(PPO / SAC)より **サンプル効率高**。**シミュレータ + Planning**(MCTS / MPC)で AlphaGo / AlphaZero / MuZero の成功。" },
  { id: "dsx-q72", category: "Optimization", difficulty: 4, question: "**Mirror Descent** の特徴として最も適切なものを選びなさい。", choices: ["Bregman Divergence を距離として一般化された勾配降下", "Newton 法", "Adam の別名", "GPU 専用"], correctIndex: 0, explanation: "**Mirror Descent**(Nemirovskii & Yudin): Bregman で空間幾何に合わせた更新。**Exponentiated Gradient**(KL Bregman)が典型。**MAB / Online Convex Opt / RL**に応用。" },
  { id: "dsx-q73", category: "Online Learning", difficulty: 4, question: "**Regret Bound** の Sublinear 性 $R_T = o(T)$ の意味として最も適切なものを選びなさい。", choices: ["平均 Regret が時間と共に 0 に収束 → No-Regret 学習", "GPU 削減", "誤差最大化", "認証"], correctIndex: 0, explanation: "**Online Learning**: Regret = 最良固定戦略との差。**$O(\\sqrt{T})$**(OGD / Hedge)・ **$O(\\log T)$**(強凸下 OGD / Online Newton Step)が代表。**Game Theory / MAB**に応用。" },
  { id: "dsx-q74", category: "Causal ML", difficulty: 4, question: "**Heterogeneous Treatment Effect(HTE)** 推定の代表手法として **誤っているもの** を選びなさい。", choices: ["Causal Forest(Wager & Athey)", "Meta-Learners(S/T/X/R/DR-Learner)", "DML + Cross-Fitting", "Bubble Sort"], correctIndex: 3, explanation: "**HTE / CATE**: Causal Forest(RF 拡張)・ **Meta-Learner**(S / T / X / R / DR)・ **DML for CATE**(Foster & Syrgkanis)。**EconML / CausalML** で実装。" },
  { id: "dsx-q75", category: "高次元", difficulty: 4, question: "**Group LASSO** の特徴として最も適切なものを選びなさい。", choices: ["変数群単位でスパース化(同群は同時に 0 or 非零)", "個別変数のみ", "全変数選択", "GPU 必須"], correctIndex: 0, explanation: "**Group LASSO**: $\\sum_g \\|\\beta_g\\|_2$ ペナルティ。**Sparse Group LASSO**(L1 + Group L1)で群内もスパース化。**Genomics / Categorical Encoding** に有効。" },
  { id: "dsx-q76", category: "Robust", difficulty: 4, question: "**Differentially Private SGD(DP-SGD)** の主要要素として最も適切なものを選びなさい。", choices: ["勾配 Clipping + Gaussian Noise 注入", "GPU 削減", "学習データ削除", "認証"], correctIndex: 0, explanation: "**DP-SGD**(Abadi et al. 2016): バッチ勾配を Clip + Noise 加算 + 累積 ε 追跡(RDP / Moments Accountant)。**フェデレーション学習 / 医療 / 金融**で標準。" },
  { id: "dsx-q77", category: "MLOps", difficulty: 4, question: "**Concept Drift vs Data Drift** の違いとして最も適切なものを選びなさい。", choices: ["Data=入力分布変化、Concept=入力 → 出力関係変化", "両者同じ", "Data=モデル変化", "Concept=データ削除"], correctIndex: 0, explanation: "**Data Drift**: $P(X)$ 変化(共変量シフト)。**Concept Drift**: $P(Y|X)$ 変化(関係性変化、再学習必要)。**Prior Drift**($P(Y)$)も。**Evidently / Arize / Fiddler** で監視。" },
  { id: "dsx-q78", category: "Foundation Models", difficulty: 4, question: "**Foundation Model 評価** の **Holistic Eval** として最も適切なものを選びなさい。", choices: ["精度 + Fairness + Robustness + Efficiency + Calibration + Toxicity の多軸評価", "精度のみ", "GPU 速度のみ", "Cost のみ"], correctIndex: 0, explanation: "**HELM**(Stanford CRFM): 多軸 Benchmark。**精度 + 公平性 + Robustness + Calibration + Toxicity + Efficiency + Bias + Memorization** の 7+ 指標で **全体評価**。" },
  { id: "dsx-q79", category: "実務", difficulty: 4, question: "**DS Expert** で重視される **研究 + 実務統合** として **誤っているもの** を選びなさい。", choices: ["最新論文 ・ 学会動向の追跡", "実データへの適切なメソッド選択", "結果の不確実性 ・ 限界の明示", "全課題に 1 つの手法のみ適用"], correctIndex: 3, explanation: "**DS Expert / Sr. DS**: ① arXiv / NeurIPS / ICML / KDD 追跡 ② 状況に応じた手法選択 ③ 不確実性 ・ 仮定の明示 ④ ステークホルダ Communication。**1 つの解決策に固執しない柔軟性**。" },
  { id: "dsx-q80", category: "実務", difficulty: 4, question: "DS エキスパート試験で重視される **高度な総合判断力** として最も適切なものを選びなさい。", choices: ["数理 + ML + 因果 + ベイズ + RL + MLOps + Foundation Model + 業務応用の統合", "数学のみ", "GUI のみ", "コードのみ"], correctIndex: 0, explanation: "**DS Expert(DSEEK)範囲**: ① 統計 ・ 因果推論 ② ML / DL / 強化学習 ③ Bayesian Modeling / PPL ④ 高次元 / Robust ⑤ Foundation Model / LLM ⑥ MLOps / Drift ⑦ 倫理 ・ プライバシ。**Sr. DS / Researcher 相当**の総合力。" },
];
