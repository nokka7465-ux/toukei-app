...
export const gradePre1Questions: Question[] = [
  ...
  { id: "pre1-q81", category: "多変量", difficulty: 4, question: "**正準相関分析(Canonical Correlation Analysis, CCA)** の用途として最も適切なものを選びなさい。", choices: ["2 つの多変量データセット間の **線形相関** の最大組合せを抽出", "次元削減", "クラスタ", "回帰"], correctIndex: 0, explanation: "**CCA**(Hotelling 1936): 2 群の変数間の相関を最大化する線形結合を求める。**遺伝子発現 × 表現型 / 脳活動 × 行動 / マルチモーダル機械学習** に応用。Deep CCA で NN 拡張も。" },
  { id: "pre1-q82", category: "多変量", difficulty: 4, question: "**判別分析(Discriminant Analysis)** で **LDA(線形)と QDA(二次)** の違いとして最も適切なものを選びなさい。", choices: ["LDA=共分散行列を全クラス共通仮定、QDA=クラス別共分散", "両者同じ", "LDA は分類不可", "QDA は回帰"], correctIndex: 0, explanation: "**LDA**(Fisher 1936): クラス間分散 / クラス内分散最大化。**QDA**: 共分散行列をクラス別に推定 → 決定境界が二次曲面。**Regularized DA / Mixture DA** で発展。" },
  { id: "pre1-q83", category: "クラスタ", difficulty: 4, question: "**Gaussian Mixture Model(GMM)** で **k-means との関係** として最も適切なものを選びなさい。", choices: ["k-means は GMM の特殊例(等分散球形 ・ Hard Assignment)", "完全別物", "GMM は離散専用", "k-means は確率モデル"], correctIndex: 0, explanation: "**GMM**: $\\sum \\pi_k N(\\mu_k, \\Sigma_k)$。**EM アルゴリズム**で推定。**k-means = $\\Sigma_k = \\sigma^2 I$ + Hard Assignment 版**。**Soft Clustering / 確率推定** が GMM の強み。" },
  { id: "pre1-q84", category: "ベイズ", difficulty: 4, question: "**Beta(α, β) 事前 + 二項尤度** の事後分布として正しいものを選びなさい。", choices: ["Beta(α + 成功数, β + 失敗数)", "正規", "ガンマ", "一様"], correctIndex: 0, explanation: "**Beta-Bernoulli 共役**: 成功 $s$ / 失敗 $f$ で事後 = Beta(α+s, β+f)。**Beta(1,1) = 一様**(Laplace's Rule of Succession)。**Bandit / A/B Test ・ 信用確率推定**で頻用。" },
  { id: "pre1-q85", category: "時系列", difficulty: 4, question: "**ARIMA(p, d, q)** の **d=1** が表すものとして正しいものを選びなさい。", choices: ["1 階差分で定常化(トレンド除去)", "AR 次数", "MA 次数", "誤差"], correctIndex: 0, explanation: "**d**: 階差回数。**d=1**: $y_t - y_{t-1}$ で定常化 → トレンド除去。**d=2**: 二階差分(加速度除去)。**SARIMA(p,d,q)(P,D,Q)_s** で季節性も。**ADF / KPSS 検定**で定常性チェック。" },
  { id: "pre1-q86", category: "時系列", difficulty: 4, question: "**自己相関関数(ACF)と偏自己相関関数(PACF)** から **AR(p)** が示唆される典型パターンを選びなさい。", choices: ["ACF: 緩やかに減衰、PACF: p 次で打ち切り", "両方打ち切り", "両方持続", "GPU"], correctIndex: 0, explanation: "**AR(p)**: ACF 減衰 ・ PACF が p 次で 0 化。**MA(q)**: ACF が q 次で 0 化 ・ PACF 減衰。**ARMA(p,q)**: 両者とも減衰。**Box-Jenkins 法**でモデル同定。" },
  { id: "pre1-q87", category: "実験計画", difficulty: 4, question: "**ラテン方格(Latin Square)計画** の用途として最も適切なものを選びなさい。", choices: ["1 つの主効果 + 2 つのブロック因子で行 ・ 列ラテン化 → 実験数削減", "全組合せ実施", "GPU", "認証"], correctIndex: 0, explanation: "**Latin Square**: $k \\times k$ の格子で各行 ・ 各列に各処理が 1 回ずつ → 実験数 $k^3 \\to k^2$。**3 因子の交互作用なし仮定**。農学 ・ 臨床試験 / 実験室で活用。" },
  { id: "pre1-q88", category: "因果推論", difficulty: 4, question: "**Backdoor 基準** の役割として最も適切なものを選びなさい。", choices: ["観察データから因果効果を識別するための共変量選択基準", "回帰", "GPU", "認証"], correctIndex: 0, explanation: "**Backdoor Criterion**(Pearl): 処置 T と結果 Y のあらゆる Backdoor Path を遮断する共変量集合 Z を見つけ、Z で調整。**Frontdoor Criterion / Do-Calculus** と並ぶ因果識別の主柱。" },
  { id: "pre1-q89", category: "高次元", difficulty: 4, question: "**カーネル PCA** の用途として最も適切なものを選びなさい。", choices: ["非線形構造の次元削減 ・ 多様体学習", "線形のみ", "GPU 不要", "認証"], correctIndex: 0, explanation: "**Kernel PCA**(Schölkopf 1998): 入力を高次元 RKHS に写し PCA → 非線形主成分。**t-SNE / UMAP / Isomap / LLE / Laplacian Eigenmap** など多様体学習の派生も多数。可視化 / 異常検知に応用。" },
  { id: "pre1-q90", category: "実務", difficulty: 4, question: "**準 1 級論述問題** で重視される点として **誤っているもの** を選びなさい。", choices: ["問題設定 → モデル選択 → 仮定明示 → 推定 → 解釈の論理一貫性", "代替手法との比較検討", "結果の限界と前提の明示", "結論を断定的に書く"], correctIndex: 3, explanation: "**準 1 級論述**(午後): 単に手法を適用するだけでなく **設定 → 仮定 → 代替検討 → 解釈 → 限界 → 提案** の論理性が問われます。**断定は不誠実**。**不確実性と仮定を明示**するのが Senior DS のスタイル。" },
];
