...
export const dsBasicQuestions: Question[] = [
  ...
  { id: "ds-q66", category: "Excel", difficulty: 2, question: "**Excel `VLOOKUP`** vs **`XLOOKUP`** の違いとして最も適切なものを選びなさい。", choices: ["XLOOKUP=左右両方検索 / Exact デフォルト / Array 返却", "両者同じ", "VLOOKUP=新", "XLOOKUP=Old"], correctIndex: 0, explanation: "**XLOOKUP**(Excel 365 2020+): VLOOKUP の進化版。**左右検索 / Exact 既定 / Array Spill / Error Default**。**INDEX + MATCH** より直感的。" },
  { id: "ds-q67", category: "Excel", difficulty: 2, question: "**ピボットテーブル** で **値フィールド** に設定できる集計関数として **誤っているもの** を選びなさい。", choices: ["合計 / 平均 / カウント", "最大 / 最小 / 標準偏差", "%累積 / 親に対する %", "GPU 利用率"], correctIndex: 3, explanation: "**Pivot 集計**: Sum / Average / Count / Max / Min / StdDev / Var / **% Running Total / % of Parent / % of Total**(値の表示形式)。GPU は関係ない。" },
  { id: "ds-q68", category: "Power Query", difficulty: 2, question: "**Power Query** の主用途として最も適切なものを選びなさい。", choices: ["GUI で取込 + 変換 + 結合 → Excel / Power BI で再利用", "値計算", "認証", "GPU"], correctIndex: 0, explanation: "**Power Query**(M Language): Excel / Power BI に標準搭載。**Self-Service ETL**。**Get Data → Transform → Load**。**Refresh** で最新データ再取得。" },
  { id: "ds-q69", category: "SQL", difficulty: 2, question: "**SQL `GROUP BY`** の用途として最も適切なものを選びなさい。", choices: ["カテゴリ列で集計 → SUM / AVG / COUNT 等", "結合", "認証", "GPU"], correctIndex: 0, explanation: "**GROUP BY**: 集計の基本。**`SELECT col, COUNT(*) FROM t GROUP BY col`**。**HAVING**(集計後フィルタ)・ **ROLLUP / CUBE / GROUPING SETS**(複合集計)も。" },
  { id: "ds-q70", category: "SQL", difficulty: 2, question: "**SQL Window 関数** の代表的用途として **誤っているもの** を選びなさい。", choices: ["ROW_NUMBER / RANK / DENSE_RANK", "LAG / LEAD / FIRST_VALUE / LAST_VALUE", "SUM OVER / AVG OVER(累計 / 移動平均)", "全行削除"], correctIndex: 3, explanation: "**Window 関数**: 集計を保ちつつ詳細行に集約値を併記。**OVER(PARTITION BY ... ORDER BY ...)**。**RANK / 累計 / 移動平均 / Top N per Group** で頻繁活用。" },
  { id: "ds-q71", category: "Python", difficulty: 2, question: "**pandas `DataFrame.describe()`** の出力として **誤っているもの** を選びなさい。", choices: ["count / mean / std", "min / 25% / 50% / 75% / max", "数値列のみ自動選択(include=\"all\" で全列)", "GPU 利用率"], correctIndex: 3, explanation: "**describe**: count / mean / std / min / quartiles / max。**include='all'** で文字列も(unique / top / freq)。**EDA の第一歩**として頻用。" },
  { id: "ds-q72", category: "BI", difficulty: 2, question: "**BI ツール** の代表として **誤っているもの** を選びなさい。", choices: ["Power BI / Tableau / Looker Studio", "Looker / Qlik / Sigma / Hex", "Metabase / Superset / Redash", "Microsoft Word"], correctIndex: 3, explanation: "**BI Tool**: Power BI / Tableau / Looker / Looker Studio / Qlik / Sigma / Hex / Metabase / Superset / Redash / Mode / Domo。**Word は文書編集**で BI ではない。" },
  { id: "ds-q73", category: "ETL", difficulty: 2, question: "**ETL vs ELT** の違いとして最も適切なものを選びなさい。", choices: ["ETL=変換後 Load、ELT=Load 後 DWH 内変換(クラウド時代の主流)", "両者同じ", "ETL=高速", "ELT=遅い"], correctIndex: 0, explanation: "**ELT**: BigQuery / Snowflake / Databricks の計算力活用 → Raw を DWH に Load → 変換は SQL / Spark で。**dbt / Dataform** が代表ツール。" },
  { id: "ds-q74", category: "Dashboard", difficulty: 2, question: "**KPI ダッシュボード設計** の基本として **誤っているもの** を選びなさい。", choices: ["重要 KPI を上部に大きく", "Drill-Down 階層構造", "色を意味で統一", "全データ単一画面詰込"], correctIndex: 3, explanation: "**ダッシュボード設計**: ① KPI Hierarchy ② Drill-Down ③ 色の意味付与 ④ 余白 ⑤ ユーザーフロー想定。**詰込はノイズ**。1 画面 1 メッセージ原則。" },
  { id: "ds-q75", category: "可視化", difficulty: 2, question: "**Pie Chart(円グラフ)** を避けるべきケースとして最も適切なものを選びなさい。", choices: ["カテゴリ数が多い(5+)・ 値が近い", "2-3 カテゴリの構成比", "100% の合計", "全構成比"], correctIndex: 0, explanation: "**Pie Chart 注意**: カテゴリ多 / 似た値 / 3D Pie は読みづらい。**横棒グラフ(Bar)推奨**。**Donut / Stacked Bar / Treemap** も検討。**Pie は 2-3 区分の構成比のみ**。" },
  { id: "ds-q76", category: "Story", difficulty: 2, question: "**Data Storytelling** の構成要素として最も適切なものを選びなさい。", choices: ["Data + Visual + Narrative の三位一体", "Data のみ", "認証", "GPU"], correctIndex: 0, explanation: "**Data Storytelling**(Brent Dykes / Cole Knaflic): Data + Visual + Narrative + Audience-Centered + Single Big Idea。**Insight 提供 + Action 促進**が目的。" },
  { id: "ds-q77", category: "Hypothesis", difficulty: 2, question: "**仮説検証型分析** の典型フローとして最も適切なものを選びなさい。", choices: ["仮説 → 指標定義 → データ準備 → 検証 → 意思決定", "データ集めて見る", "認証", "GPU"], correctIndex: 0, explanation: "**仮説検証**: イシューツリー / Logic Tree で課題分解 → KPI 定義 → 分析 → アクション。**Exploratory(EDA)+ Confirmatory(検証)** の両輪。" },
  { id: "ds-q78", category: "AB Test", difficulty: 2, question: "**A/B Test** で **チェックすべき事前項目** として **誤っているもの** を選びなさい。", choices: ["仮説 + 評価指標(OEC)", "サンプルサイズ + 期間", "ランダム化 + Stratification", "事後 p-hacking"], correctIndex: 3, explanation: "**A/B Test 計画**: 仮説 / Metric / Sample Size / Duration / Randomization / Pre-registration。**事後解釈 ・ Cherry-pick(p-hacking)は禁忌**。" },
  { id: "ds-q79", category: "Privacy", difficulty: 2, question: "**個人情報を扱うデータ分析** の基本原則として **誤っているもの** を選びなさい。", choices: ["最小権限 / 必要最小限取得", "匿名化 / 仮名化 / 集約", "目的明示 + 同意 + 監査ログ", "全社員に Read 権限"], correctIndex: 3, explanation: "**Privacy by Design**: 最小権限 / 匿名化 / 同意 / Audit。**個情法 / GDPR / CCPA**。**全社員 Read は重大違反**(機密漏洩 ・ ロール定義不在)。" },
  { id: "ds-q80", category: "Cert", difficulty: 1, question: "**DS 基礎(統計検定 データサイエンス基礎)** の特徴として最も適切なものを選びなさい。", choices: ["Excel 中心 / 実データ分析 + 統計検定が共通の合格証", "Python 必須", "GUI のみ", "数学のみ"], correctIndex: 0, explanation: "**DS 基礎**: 統計検定 ・ 大学入試センターと共通。**Excel ベースで実データ分析 / EDA / 検定 / 回帰**を出題。**DS リテラシー / DS 発展 / DS エキスパート** へ階層的に発展。" },
];
