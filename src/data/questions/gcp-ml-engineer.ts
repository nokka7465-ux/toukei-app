...
export const gcpMlEngineerQuestions: Question[] = [
  ...
  { id: "gcpml-q66", category: "Vertex AI", difficulty: 3, question: "**Vertex AI Training** の Custom Job 設定として **誤っているもの** を選びなさい。", choices: ["Container Image(Prebuilt / Custom)", "Machine Type / GPU / TPU", "Distributed Training(MultiWorker / Parameter Server)", "GPU 個人所有必須"], correctIndex: 3, explanation: "**Vertex Training**: Prebuilt(TF / PyTorch / sklearn / XGB)or Custom Container ・ Worker Pool(MirroredStrategy / MultiWorker / ParameterServer / Reduction Server)・ GPU / TPU 選択。" },
  { id: "gcpml-q67", category: "Vertex AI", difficulty: 3, question: "**Vertex AI Pipelines** で **Component 定義** に推奨される SDK として最も適切なものを選びなさい。", choices: ["Kubeflow Pipelines(KFP)v2 + TFX", "Airflow DAG", "GUI のみ", "Excel"], correctIndex: 0, explanation: "**Vertex Pipelines**: **KFP v2**(Python @component / @pipeline decorator)・ TFX Component 互換 ・ **CustomTrainingJobOp / EndpointOp** 等のプリビルト ・ ML Metadata 自動記録。" },
  { id: "gcpml-q68", category: "Feature Store", difficulty: 3, question: "**Vertex AI Feature Store v2** の特徴として最も適切なものを選びなさい。", choices: ["BigQuery バックエンド + Online Store(Bigtable)・ Point-in-Time 整合", "オンプレ専用", "GPU 必須", "RDBMS のみ"], correctIndex: 0, explanation: "**Feature Store v2**(2024): BigQuery を Source-of-Truth ・ Online Store(Bigtable ベース)で低レイテンシ ・ **Feature View / Online Serving**・ **Point-in-Time Correctness** で訓練 / 推論整合。" },
  { id: "gcpml-q69", category: "Vertex AI", difficulty: 3, question: "**Vertex AI Vizier** が提供する最適化として最も適切なものを選びなさい。", choices: ["Black-Box ハイパーパラメータ最適化(Bayesian / Bandit / Grid / Random)", "GPU 学習", "認証", "ETL"], correctIndex: 0, explanation: "**Vizier**(Google 社内発): Bayesian Optimization Service。**Hyperparameter Tuning Job** の裏側 ・ **Conditional Search Space / Parallelism / Early Stopping** 対応。" },
  { id: "gcpml-q70", category: "Endpoint", difficulty: 3, question: "**Vertex AI Endpoint** の Traffic Split 設定で **A/B Test** を実現する例として最も適切なものを選びなさい。", choices: ["Deploy v1 90%, v2 10% → メトリクス比較で 100/0 切替", "v1 のみ", "全削除", "GPU 共有"], correctIndex: 0, explanation: "**Vertex Endpoint Traffic Split**: 同一 Endpoint に複数 Model Deploy → traffic_percentage で Canary。**Auto Scaling**(min/max replicas)・ **Private Endpoint**(VPC 内)も。" },
  { id: "gcpml-q71", category: "Distributed", difficulty: 3, question: "**Vertex AI Reduction Server** の役割として最も適切なものを選びなさい。", choices: ["AllReduce 専用 VM → GPU 帯域節約 → 学習加速", "Training Worker", "認証", "GUI"], correctIndex: 0, explanation: "**Reduction Server**: GPU ノードとは別の集約専用 VM。**Network 50%+ 削減** + 大規模分散学習で 30-50% 加速。**Megatron / DeepSpeed** との組合せも。" },
  { id: "gcpml-q72", category: "AutoML", difficulty: 3, question: "**AutoML Tabular** の評価指標として **誤っているもの** を選びなさい。", choices: ["RMSE / MAE(Regression)", "AUC PR / ROC(Classification)", "Forecasting MAPE / Quantile Loss", "GPU 利用率"], correctIndex: 3, explanation: "**AutoML Tabular Metrics**: Regression(RMSE / MAE / R²)・ Classification(AUC / Log Loss / F1)・ Forecasting(WAPE / MAPE / Quantile)。GPU は計算リソースで指標ではない。" },
  { id: "gcpml-q73", category: "GenAI", difficulty: 3, question: "**Vertex AI Model Garden** の特徴として最も適切なものを選びなさい。", choices: ["100+ Foundation Model(Google / Anthropic / Meta / OSS)を統一インタフェースで提供", "GPU 直販", "Anthropic 専用", "GUI のみ"], correctIndex: 0, explanation: "**Model Garden**: Gemini / Imagen / Veo + Claude(Anthropic)+ Llama / Mistral / Gemma / Falcon 等の OSS。**Deploy / Tune / Notebook**を統一 UX で。" },
  { id: "gcpml-q74", category: "GenAI", difficulty: 3, question: "**Vertex AI Gemini API** で **Context Caching** の特徴として最も適切なものを選びなさい。", choices: ["長文 Prompt をキャッシュ → 再利用時 75% コスト削減 + 低レイテンシ", "速度低下", "GPU 必須", "認証"], correctIndex: 0, explanation: "**Context Caching**(Gemini 1.5+): 同 Prompt 前半を Cache → 後続呼出の Input Token を **75% off**。**TTL 1 時間(変更可)**・ Multi-turn / RAG / Agent で効果大。" },
  { id: "gcpml-q75", category: "Agent", difficulty: 3, question: "**Vertex AI Agent Builder**(旧 Gen App Builder)の機能として **誤っているもの** を選びなさい。", choices: ["Vertex AI Search(RAG)", "Conversational Agent(Dialogflow CX 統合)", "Agentic Workflow(2024+)", "GPU 提供"], correctIndex: 3, explanation: "**Agent Builder**: Search(RAG)+ Conversational Agent + Agentic Workflow + Tools 統合。**Low-Code で会話型 AI / Agent 構築**。Microsoft Copilot Studio に対応。" },
  { id: "gcpml-q76", category: "Responsible AI", difficulty: 3, question: "**Vertex Explainable AI** の手法として最も適切なものを選びなさい。", choices: ["Sampled Shapley(Tabular)/ Integrated Gradients(Image / Text)/ XRAI(Image)", "Random Forest", "GPU 速度", "認証"], correctIndex: 0, explanation: "**Vertex XAI**: Sampled Shapley(Tabular)・ Integrated Gradients(DL)・ XRAI(画像領域)。**AutoML / Custom 両対応**。**Feature Attribution Drift Monitoring** とも統合。" },
  { id: "gcpml-q77", category: "MLOps", difficulty: 3, question: "**Vertex AI Model Monitoring** で監視可能な指標として **誤っているもの** を選びなさい。", choices: ["Training-Serving Skew", "Prediction Drift", "Feature Attribution Drift", "GPU Drift"], correctIndex: 3, explanation: "**Vertex Model Monitoring**: Training vs Serving の **Skew**(分布差)・ **Drift**(本番時間変化)・ **Feature Attribution Drift**(説明変化)。**Schedule + Alert + BigQuery Export**。" },
  { id: "gcpml-q78", category: "Security", difficulty: 3, question: "**Vertex AI VPC Service Controls** の役割として最も適切なものを選びなさい。", choices: ["GCP サービスの Perimeter で Data Exfiltration 防止", "認証 SSO", "GPU 監視", "Cost 削減"], correctIndex: 0, explanation: "**VPC SC**: 組織レベルで GCP サービスのアクセス境界定義。**Vertex AI + BigQuery + GCS** を Perimeter 化 ・ 外部への移送 / コピー防止。**Customer-Managed Encryption Keys(CMEK)** と組合せ。" },
  { id: "gcpml-q79", category: "Cost", difficulty: 3, question: "**Vertex AI のコスト最適化** として **誤っているもの** を選びなさい。", choices: ["Spot(Preemptible)VM + Checkpoint", "TPU vs GPU の使い分け", "Batch Prediction(50% off)", "全 Endpoint Always-on Max replicas"], correctIndex: 3, explanation: "**Cost 最適化**: Spot + Checkpoint / TPU(Transformer 系で効率)/ Batch Prediction(オフライン 50% 安)/ Min Replicas = 0(Auto Scale)/ Context Caching / Model Distillation。" },
  { id: "gcpml-q80", category: "Cert", difficulty: 2, question: "**Professional ML Engineer** の試験対象として最も適切なものを選びなさい。", choices: ["業務 ML Engineer / Data Scientist(2 年以上 + GCP 1 年以上経験推奨)", "未経験", "Cloud Practitioner", "Excel 利用者"], correctIndex: 0, explanation: "**Pro MLE**: 業界経験 3+ 年 ・ GCP 1+ 年推奨 ・ 60 問 ・ 2 時間。**Vertex AI / TFX / BigQuery / Dataflow / MLOps / Responsible AI** が中核。**Associate → Pro** の階層。" },
];
