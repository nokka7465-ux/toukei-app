...
export const mathBasicsQuestions: Question[] = [
  ...
  { id: "math-q66", category: "線形代数", difficulty: 3, question: "**行列の階数(rank)** の意味として最も適切なものを選びなさい。", choices: ["線形独立な列(または行)の最大個数", "対角成分の和", "最大成分", "行数"], correctIndex: 0, explanation: "**rank**: 列空間 / 行空間の次元(列ランク=行ランク)。**フルランク**(rank = min(m,n))・ **rank 落ち**(冗長な列)・ **rank-nullity 定理**(rank + nullity = n)。線形回帰の正則性条件。" },
  { id: "math-q67", category: "線形代数", difficulty: 3, question: "**直交行列 $Q$** の特徴として **誤っているもの** を選びなさい。", choices: ["$Q^T Q = I$", "$\\det Q = \\pm 1$", "ノルム保存 $\\|Qx\\| = \\|x\\|$", "$Q = 2I$"], correctIndex: 3, explanation: "**直交行列**: $Q^T = Q^{-1}$ ・ $\\det = \\pm 1$ ・ ノルム / 角度保存。**回転 + 反射**を表現。**QR 分解 / 主成分分析 / グラム ・ シュミット**で多用。" },
  { id: "math-q68", category: "微積分", difficulty: 3, question: "**連鎖律(Chain Rule)** の式として正しいものを選びなさい。", choices: ["$\\frac{df(g(x))}{dx} = f'(g(x)) \\cdot g'(x)$", "$f(g(x))'$", "$f(x) + g(x)$", "$x^2$"], correctIndex: 0, explanation: "**Chain Rule**: 合成関数の微分。**ニューラルネット Backpropagation** の数学的基礎(計算グラフを逆向きに連鎖律で微分)。多変数版: $\\partial L / \\partial w = \\sum (\\partial L / \\partial z_i)(\\partial z_i / \\partial w)$。" },
  { id: "math-q69", category: "微積分", difficulty: 3, question: "**勾配(Gradient)** $\\nabla f$ の意味として最も適切なものを選びなさい。", choices: ["各変数の偏微分を並べたベクトル ・ 関数が最も急増する方向", "スカラー値", "二階微分", "積分値"], correctIndex: 0, explanation: "**$\\nabla f = (\\partial f / \\partial x_1, \\ldots, \\partial f / \\partial x_n)$**。最急上昇方向 → **逆方向に動かすのが Gradient Descent**。**Jacobian**(ベクトル関数の勾配行列)・ **Hessian**(2 階)も。" },
  { id: "math-q70", category: "微積分", difficulty: 3, question: "**ヤコビアン(Jacobian)** $J$ の用途として最も適切なものを選びなさい。", choices: ["多変数関数の 1 階偏微分を並べた行列 ・ 変数変換 / Backprop / Newton 法に必須", "対角行列", "Hessian と同じ", "ベクトル"], correctIndex: 0, explanation: "**Jacobian**: $J_{ij} = \\partial f_i / \\partial x_j$。**変数変換の積分**(`|det J|`)・ **Reverse-mode AD**(Backprop = Jacobian-Vector Product)・ Inverse Problem に。" },
  { id: "math-q71", category: "確率", difficulty: 3, question: "**期待値の線形性** $E[aX + bY]$ が表す式として正しいものを選びなさい。", choices: ["$aE[X] + bE[Y]$(X, Y が独立でなくても成立)", "$E[X] \\cdot E[Y]$", "$E[X^2]$", "$0$"], correctIndex: 0, explanation: "**期待値の線形性**: 独立性不問。**分散は独立性必要**($Var[X+Y] = Var[X] + Var[Y]$ は独立時のみ、一般は $+ 2Cov[X, Y]$)。" },
  { id: "math-q72", category: "確率", difficulty: 3, question: "**条件付き確率** $P(A|B) = ?$ の式として最も適切なものを選びなさい。", choices: ["$P(A \\cap B) / P(B)$", "$P(A) + P(B)$", "$P(A) \\cdot P(B)$", "$P(A) - P(B)$"], correctIndex: 0, explanation: "**Conditional**: $P(A|B) = P(A \\cap B) / P(B)$($P(B) > 0$)。**乗法定理**: $P(A \\cap B) = P(A|B) P(B)$。**独立**: $P(A|B) = P(A)$。**ベイズの定理**の構成要素。" },
  { id: "math-q73", category: "統計", difficulty: 3, question: "**標本平均 $\\bar{X}$ の分散** として正しいものを選びなさい(独立 ・ 等分散 $\\sigma^2$)。", choices: ["$\\sigma^2 / n$", "$n \\sigma^2$", "$\\sigma^2$", "$0$"], correctIndex: 0, explanation: "**$Var[\\bar{X}] = \\sigma^2 / n$**($n$ で割る)→ **標本サイズ増で分散減**(中心極限定理に通ず)。**標準誤差 $SE = \\sigma / \\sqrt{n}$**。" },
  { id: "math-q74", category: "統計", difficulty: 3, question: "**中心極限定理(CLT)** の主張として最も適切なものを選びなさい。", choices: ["独立同分布の標本平均は $n \\to \\infty$ で正規分布に従う", "全分布は正規", "$\\bar{X} = \\mu$", "分散ゼロ"], correctIndex: 0, explanation: "**CLT**: $\\sqrt{n}(\\bar{X} - \\mu) / \\sigma \\to N(0, 1)$ in distribution。**元の分布形に依らず**正規へ収束(分散有限が条件)。推定 ・ 検定の理論基盤。" },
  { id: "math-q75", category: "最適化", difficulty: 3, question: "**Lagrange 関数** $L(x, \\lambda) = f(x) - \\lambda g(x)$ の用途として最も適切なものを選びなさい。", choices: ["等式制約 $g(x)=0$ つき最適化問題を無制約に変換", "GPU 演算", "認証", "DB クエリ"], correctIndex: 0, explanation: "**Lagrange**: 等式制約 → 1 階条件 $\\nabla L = 0$。**KKT 条件**(不等式拡張)が SVM / Constrained Optimization の理論基盤。**双対問題**(SVM の Soft Margin)で活躍。" },
  { id: "math-q76", category: "最適化", difficulty: 3, question: "**Newton 法** の特徴として最も適切なものを選びなさい。", choices: ["二次収束(Hessian 利用)・ 1 階+2 階微分必要", "1 次収束", "Hessian 不要", "GPU 必須"], correctIndex: 0, explanation: "**Newton**: $x_{t+1} = x_t - H^{-1} \\nabla f$。**二次収束**(GD の 1 次より遥かに高速)だが Hessian 計算コスト大。**Quasi-Newton(L-BFGS)** で近似し実用化。" },
  { id: "math-q77", category: "情報理論", difficulty: 3, question: "**Cross-Entropy** $H(p, q)$ の意味として最も適切なものを選びなさい。", choices: ["$H(p, q) = -\\sum p(x) \\log q(x)$ ・ q を p に近づける指標", "ベクトル長", "GPU 値", "認証コスト"], correctIndex: 0, explanation: "**Cross-Entropy**: $H(p, q) = H(p) + D_{KL}(p \\| q)$。**ML の分類損失で標準**。q(予測分布)を p(真の分布)に近づける。**Negative Log Likelihood** と等価。" },
  { id: "math-q78", category: "情報理論", difficulty: 3, question: "**Mutual Information** $I(X; Y)$ の意味として最も適切なものを選びなさい。", choices: ["X と Y の共有情報量 ・ 独立なら 0", "GPU 数", "認証", "Cost"], correctIndex: 0, explanation: "**MI**: $I(X;Y) = H(X) - H(X|Y) = D_{KL}(p(x,y) \\| p(x)p(y))$。**特徴量選択 / 表現学習 / InfoGAN / VAE / Contrastive Learning** で多用。非線形相関も捉える。" },
  { id: "math-q79", category: "凸最適化", difficulty: 3, question: "**強凸関数** の特徴として最も適切なものを選びなさい。", choices: ["凸 + 2 次補助項 → 一意最小値 + 線形収束保証", "極大値多数", "GPU 必須", "認証"], correctIndex: 0, explanation: "**Strongly Convex**: $f(y) \\geq f(x) + \\nabla f(x)^T(y-x) + (\\mu/2)\\|y-x\\|^2$。**勾配降下が線形収束**(誤差が幾何級数的減少)。Ridge / SVM 損失が代表例。" },
  { id: "math-q80", category: "応用", difficulty: 3, question: "**Stochastic Gradient Descent(SGD)** の利点として **誤っているもの** を選びなさい。", choices: ["大規模データで Full Batch GD より高速", "Mini-batch でメモリ効率良", "ノイズで局所最適脱出", "常に Full Batch より精度高"], correctIndex: 3, explanation: "**SGD**: ミニバッチで近似勾配 → 高速 ・ メモリ効率 ・ Implicit Regularization。**Full Batch より速いが精度は条件依存**。**Momentum / Adam / AdamW**で改良。NN 学習の標準。" },
];
