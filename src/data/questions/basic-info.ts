...
export const basicInfoQuestions: Question[] = [
  ...
  { id: "fe-q66", category: "アルゴリズム", difficulty: 3, question: "**二分探索木(BST)** の **平均探索時間計算量** として最も適切なものを選びなさい。", choices: ["$O(\\log n)$", "$O(n)$", "$O(n^2)$", "$O(1)$"], correctIndex: 0, explanation: "**BST 平均**: $O(\\log n)$ (平衡な場合)。**最悪は $O(n)$**(片寄り)。**AVL / Red-Black Tree / B-Tree** で平衡を保証。**Hash は平均 $O(1)$** だが順序なし。" },
  { id: "fe-q67", category: "アルゴリズム", difficulty: 3, question: "**マージソート** と **クイックソート** の比較として最も適切なものを選びなさい。", choices: ["Merge: $O(n\\log n)$ 安定 + 追加メモリ、Quick: 平均 $O(n\\log n)$ in-place + 不安定", "両者同じ", "Merge=$O(n^2)$", "Quick=$O(1)$"], correctIndex: 0, explanation: "**Merge Sort**: 分割統治 ・ 安定 ・ $O(n)$ 追加メモリ ・ 最悪 $O(n\\log n)$。**Quick**: in-place ・ 不安定 ・ 平均 $O(n\\log n)$ / 最悪 $O(n^2)$。**Timsort**(Python / Java)が実用標準。" },
  { id: "fe-q68", category: "アルゴリズム", difficulty: 3, question: "**Dijkstra 法** の用途として最も適切なものを選びなさい。", choices: ["単一始点最短経路(非負重みグラフ)", "全点間最短", "最大流", "最小全域木"], correctIndex: 0, explanation: "**Dijkstra**: 始点から各頂点への最短(非負重み)・ Priority Queue で $O((V+E)\\log V)$。**Bellman-Ford**(負重み可)・ **Floyd-Warshall**(全点間)・ **A***(ヒューリスティクス)。" },
  { id: "fe-q69", category: "データ構造", difficulty: 3, question: "**ハッシュテーブル** の **衝突解決法** として **誤っているもの** を選びなさい。", choices: ["チェイン法", "オープンアドレス法(線形探査 / 二重ハッシュ)", "Cuckoo Hashing", "Bubble Sort"], correctIndex: 3, explanation: "**衝突解決**: チェイン法(連結リスト)・ オープンアドレス(線形 / 二重 / 二乗探査)・ Cuckoo Hashing。**Bubble Sort はソート**で無関係。Load Factor を 0.7 程度に保つ。" },
  { id: "fe-q70", category: "ネットワーク", difficulty: 3, question: "**OSI 参照モデル** の 7 階層として **誤っているもの** を選びなさい。", choices: ["物理 / データリンク / ネットワーク", "トランスポート / セッション / プレゼンテーション", "アプリケーション", "GPU 層"], correctIndex: 3, explanation: "**OSI 7 層**: ① 物理 ② DL ③ NW(IP)④ Trans(TCP/UDP)⑤ Session ⑥ Presentation ⑦ Application。**TCP/IP 4 層モデル**(Link / Internet / Trans / Application)が実用。" },
  { id: "fe-q71", category: "ネットワーク", difficulty: 3, question: "**TCP** と **UDP** の主な違いとして最も適切なものを選びなさい。", choices: ["TCP=コネクション + 順序保証、UDP=コネクションレス + 高速", "両者同じ", "UDP は信頼性高", "TCP は速度優先"], correctIndex: 0, explanation: "**TCP**: 3-way Handshake / 順序保証 / 再送 / 輻輳制御。Web / Mail / File 転送。**UDP**: 高速 ・ シンプル ・ 信頼性なし。**DNS / 音声 / ストリーミング / DHCP**。HTTP/3 は UDP ベース(QUIC)。" },
  { id: "fe-q72", category: "DB", difficulty: 3, question: "**SQL の `JOIN` の種類** として **誤っているもの** を選びなさい。", choices: ["INNER / LEFT / RIGHT / FULL OUTER", "CROSS JOIN(直積)", "SELF JOIN(自己結合)", "GPU JOIN"], correctIndex: 3, explanation: "**JOIN**: INNER(共通)・ LEFT OUTER(左全)・ RIGHT OUTER(右全)・ FULL OUTER(両全)・ CROSS(直積)・ SELF(同一テーブル結合)。GPU JOIN は無関係。" },
  { id: "fe-q73", category: "DB", difficulty: 3, question: "**ACID 特性** として **誤っているもの** を選びなさい。", choices: ["Atomicity(原子性)", "Consistency(一貫性)", "Isolation(独立性)", "Distribution(分散性)"], correctIndex: 3, explanation: "**ACID**: Atomicity / Consistency / Isolation / **Durability**(永続性)。NoSQL では **BASE**(Basic Availability / Soft state / Eventually consistent)も。" },
  { id: "fe-q74", category: "セキュリティ", difficulty: 3, question: "**SQL Injection** 対策として最も適切なものを選びなさい。", choices: ["プリペアドステートメント / Parameterized Query", "GUI のみ", "GPU 増設", "全 SQL を平文で保存"], correctIndex: 0, explanation: "**SQLi 対策**: ① **Prepared Statement / Bind 変数** が最強 ② Input Validation / Escape ③ 最小権限 DB User ④ WAF ⑤ ORM 使用。**Bobby Tables**(XKCD)が古典例。" },
  { id: "fe-q75", category: "セキュリティ", difficulty: 3, question: "**OWASP Top 10** に含まれる代表的 Web 脆弱性として **誤っているもの** を選びなさい。", choices: ["XSS(Cross-Site Scripting)", "CSRF(Cross-Site Request Forgery)", "Broken Access Control", "GPU Overflow"], correctIndex: 3, explanation: "**OWASP Top 10**: Broken Access Control / Crypto Failures / Injection(XSS 含)/ Insecure Design / Misconfiguration / SSRF / 認証 ・ Identification 失敗 / Software & Data Integrity / Logging & Monitoring 失敗 / Vulnerable Components。" },
  { id: "fe-q76", category: "OS", difficulty: 3, question: "**プロセス** と **スレッド** の違いとして最も適切なものを選びなさい。", choices: ["プロセス=独立メモリ、スレッド=メモリ共有 + 軽量", "両者同じ", "プロセス=軽量", "スレッド=独立メモリ"], correctIndex: 0, explanation: "**Process**: 独立メモリ空間 ・ 切替コスト高。**Thread**: メモリ共有 + 軽量切替。**Context Switch / Race Condition / Mutex / Semaphore**。Python は GIL で並列性制限。" },
  { id: "fe-q77", category: "開発", difficulty: 3, question: "**Git** の基本コマンドとして **誤っているもの** を選びなさい。", choices: ["clone / add / commit / push / pull", "branch / checkout / merge / rebase", "stash / cherry-pick / reset / revert", "compile / link / debug"], correctIndex: 3, explanation: "**Git**: 分散 VCS。**compile / link / debug** はビルドツール(gcc / make 等)。**GitHub / GitLab / Bitbucket** が代表ホスティング。**Pull Request / Code Review** が現代開発の標準。" },
  { id: "fe-q78", category: "開発", difficulty: 3, question: "**TDD(Test-Driven Development)** の基本サイクルとして最も適切なものを選びなさい。", choices: ["Red(失敗テスト)→ Green(最小実装)→ Refactor(改善)", "実装 → テスト → 削除", "GUI のみ", "GPU 利用"], correctIndex: 0, explanation: "**TDD**(Kent Beck): Red / Green / Refactor の短サイクル。**Unit Test(jest / pytest / JUnit)** が基本。**BDD**(Behavior-Driven)・ **ATDD**(Acceptance)で発展。" },
  { id: "fe-q79", category: "AI", difficulty: 3, question: "**機械学習の典型ワークフロー** として最も適切なものを選びなさい。", choices: ["問題定義 → データ収集 → 前処理 → 特徴量 → モデル → 評価 → デプロイ", "コーディングのみ", "GUI 操作のみ", "GPU 増設のみ"], correctIndex: 0, explanation: "**ML ワークフロー(CRISP-DM / OSEMN)**: Business 理解 → Data 理解 / 前処理 → Modeling → Evaluation → Deployment。**反復的プロセス** で MLOps 観点も加味。" },
  { id: "fe-q80", category: "実務", difficulty: 1, question: "基本情報技術者試験で重視される **実装基礎能力** として最も適切なものを選びなさい。", choices: ["アルゴリズム + データ構造 + OS + DB + NW + SEC + 開発手法 + AI / クラウド基礎の総合", "プログラミング言語 1 つだけ", "GUI 操作のみ", "数学のみ"], correctIndex: 0, explanation: "**FE 試験範囲**(IPA レベル 2): ① 基礎理論 ② アルゴリズム + プログラミング ③ コンピュータシステム ・ NW ・ DB ④ セキュリティ ⑤ 開発 / マネジメント ⑥ ストラテジ。**エンジニア入門の事実上標準**。" },
];
