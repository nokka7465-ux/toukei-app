...
export const dbSpecialistQuestions: Question[] = [
  ...
  { id: "db-q66", category: "Partition", difficulty: 3, question: "**水平パーティショニング** の典型方式として **誤っているもの** を選びなさい。", choices: ["Range(日付 / ID 範囲)", "Hash(キー Hash 値)", "List(値リスト)", "GPU パーティション"], correctIndex: 3, explanation: "**水平パーティション**: Range / Hash / List / Composite。**メリット**: スキャン削減 / 並列処理 / 古い Partition の高速削除。**垂直パーティション**(列分割)もあり用途別。" },
  { id: "db-q67", category: "Index", difficulty: 3, question: "**Covering Index(Included Column)** の利点として最も適切なものを選びなさい。", choices: ["Index だけでクエリ完結(Table Lookup 不要)→ I/O 削減", "Table 不要", "GPU 高速", "認証"], correctIndex: 0, explanation: "**Covering Index**: WHERE / JOIN / SELECT 全列を含む → **Index Only Scan**。**SQL Server INCLUDE / PostgreSQL INCLUDE / Oracle Function-Based Index** で対応。" },
  { id: "db-q68", category: "Index", difficulty: 3, question: "**Bitmap Index** の利点と制約として最も適切なものを選びなさい。", choices: ["低 Cardinality 列で高速 + AND/OR 高効率、更新コスト高 / OLTP 不向き", "OLTP 向き", "高 Cardinality 専用", "GPU 必須"], correctIndex: 0, explanation: "**Bitmap**: 低 Cardinality(性別 / 状態フラグ)・ Bit 演算で高速複合検索 → DWH 向き。**Oracle DWH / 列ストア DB** で活用。**OLTP では更新ロック競合**で B-Tree 推奨。" },
  { id: "db-q69", category: "Optimizer", difficulty: 4, question: "**Optimizer Hint** の役割として最も適切なものを選びなさい。", choices: ["Optimizer の選択を上書き(特定 Index / Join 順序強制)", "全自動", "GUI", "認証"], correctIndex: 0, explanation: "**Hint**: Oracle `/*+ INDEX */ / PostgreSQL pg_hint_plan / SQL Server OPTION`。**統計が誤った場合の応急対応**。基本は **統計更新 / Plan Stability 確保** が正攻法。" },
  { id: "db-q70", category: "Locking", difficulty: 4, question: "**MVCC(Multi-Version Concurrency Control)** の利点として最も適切なものを選びなさい。", choices: ["Reader は Writer をブロックしない(スナップショット隔離)", "全ロック", "GPU 並列", "認証"], correctIndex: 0, explanation: "**MVCC**: PostgreSQL / Oracle / Spanner / TiDB。**Read Committed / Repeatable Read / Serializable Snapshot Isolation**。**MySQL InnoDB**(MVCC + GAP Lock で Phantom 防止)。" },
  { id: "db-q71", category: "Replication", difficulty: 4, question: "**Read Replica** の典型用途として **誤っているもの** を選びなさい。", choices: ["読込負荷分散", "DR(Disaster Recovery)候補", "Reporting / BI 別系統化", "Write スループット向上"], correctIndex: 3, explanation: "**Read Replica**: 読込分散 ・ DR ・ Reporting 分離 ・ Cross-Region。**Write は Primary 単一**で Replica は読込専用。**Multi-Master**(Galera / Aurora Multi-Master / CockroachDB)で Write スケール可。" },
  { id: "db-q72", category: "Sharding", difficulty: 4, question: "**Sharding** の代表的分散方式として **誤っているもの** を選びなさい。", choices: ["Range Sharding", "Hash Sharding", "Directory(Lookup Table)", "GPU Sharding"], correctIndex: 3, explanation: "**Sharding**: Range(範囲)・ Hash(均等)・ Directory(ルックアップ)・ Geo(地理)。**ホットスポット / Resharding コスト**が課題。**MongoDB / Cassandra / Vitess / TiDB**で実装。" },
  { id: "db-q73", category: "BigData", difficulty: 4, question: "**列指向 DB(Columnar Database)** の利点として最も適切なものを選びなさい。", choices: ["分析クエリ高速 + 高圧縮率(同型値が連続)", "OLTP 高速", "GPU 必須", "認証"], correctIndex: 0, explanation: "**Columnar**: 列単位ストレージ → **Scan / Aggregate 高速 + 圧縮高(Run-Length / Dictionary / Bit-Packing)**。**Redshift / BigQuery / Snowflake / ClickHouse / DuckDB / Vertica**。" },
  { id: "db-q74", category: "Lakehouse", difficulty: 4, question: "**Delta Lake / Apache Iceberg / Hudi** の共通機能として最も適切なものを選びなさい。", choices: ["S3 / GCS 上で ACID Tx + Time Travel + Schema Evolution + Upsert", "OLTP", "GPU", "認証"], correctIndex: 0, explanation: "**Open Table Format**: ACID / Time Travel / Schema/Partition Evolution / MERGE。**Lakehouse**(Bronze/Silver/Gold)の基盤。Databricks(Delta)・ Snowflake / AWS / Azure(Iceberg)対応。" },
  { id: "db-q75", category: "NoSQL", difficulty: 4, question: "**Vector Database** の代表用途として最も適切なものを選びなさい。", choices: ["Embedding 検索(類似度 ・ ANN)・ RAG / 推薦", "OLTP", "正規化", "ETL"], correctIndex: 0, explanation: "**Vector DB**: Pinecone / Weaviate / Milvus / Qdrant / **pgvector / Redis Vector / Elasticsearch dense_vector**。**HNSW / IVF-PQ / DiskANN** インデックス。RAG の中核。" },
  { id: "db-q76", category: "Security", difficulty: 4, question: "**TDE(Transparent Data Encryption)** の特徴として最も適切なものを選びなさい。", choices: ["DB ファイル / バックアップ レベルでアプリ透過に暗号化", "アプリ修正必要", "GPU 暗号化", "認証 SSO"], correctIndex: 0, explanation: "**TDE**: Oracle / SQL Server / Aurora / RDS。**Always Encrypted / Column-Level Encryption / Application-Level Encryption** とのトレードオフ。**KMS / HSM** と連携。" },
  { id: "db-q77", category: "Audit", difficulty: 4, question: "**DB 監査ログ** の典型対象として **誤っているもの** を選びなさい。", choices: ["DDL(CREATE / DROP / ALTER)", "DCL(GRANT / REVOKE)", "機密 Table への DML / SELECT", "GPU 使用率"], correctIndex: 3, explanation: "**DB 監査**: DDL / DCL / 機密データアクセス / 失敗ログイン / 権限変更。**Oracle Audit Vault / SQL Server SQL Audit / PG Audit / pgAudit**。**規制(SOX / HIPAA / PCI-DSS)** 対応必須。" },
  { id: "db-q78", category: "Cloud DB", difficulty: 4, question: "**Cloud マネージド DB**(RDS / Aurora / CloudSQL / Azure SQL)の利点として **誤っているもの** を選びなさい。", choices: ["バックアップ / パッチ自動化", "Multi-AZ 高可用性", "Read Replica / Failover マネージド", "完全に PostgreSQL 互換のみ"], correctIndex: 3, explanation: "**Managed DB**: 運用負荷大幅削減 ・ HA / DR / Patch / Backup 自動。MySQL / PostgreSQL / Oracle / SQL Server / MariaDB / **Aurora**(クラウドネイティブ)等多様。" },
  { id: "db-q79", category: "Modern Data", difficulty: 4, question: "**HTAP(Hybrid Transactional/Analytical Processing)** の代表的実装として **誤っているもの** を選びなさい。", choices: ["TiDB / Spanner / CockroachDB", "SAP HANA / Singlestore", "Snowflake Unistore / Aurora + Zero-ETL", "Bubble Sort DB"], correctIndex: 3, explanation: "**HTAP**: OLTP + OLAP を 1 DB で実現。**TiDB / Spanner / CockroachDB / SAP HANA / Singlestore / Aurora Limitless / Snowflake Unistore**。ETL なしで分析。" },
  { id: "db-q80", category: "実務", difficulty: 3, question: "DB スペシャリストで重視される **総合 DB 設計 + 運用力** として最も適切なものを選びなさい。", choices: ["概念 / 論理 / 物理設計 + 性能 / セキュリティ / 可用性 / コスト / NoSQL / Lakehouse の統合", "コードのみ", "GUI のみ", "Excel のみ"], correctIndex: 0, explanation: "**DB SP 試験範囲**: ① 概念 ・ 論理 ・ 物理設計 ② 性能チューニング ③ 障害対策 ・ 復旧 ④ セキュリティ ⑤ 運用管理 ⑥ NoSQL / NewSQL / Lakehouse。**IPA レベル 4 高度試験**。" },
];
