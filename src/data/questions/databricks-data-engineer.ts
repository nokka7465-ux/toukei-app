...
export const databricksDataEngineerQuestions: Question[] = [
  ...
  { id: "dbx-q66", category: "Delta Lake", difficulty: 3, question: "**Delta Lake の Transaction Log(_delta_log)** の役割として最も適切なものを選びなさい。", choices: ["JSON / Parquet で変更を順次記録 → ACID + Time Travel + Schema Evolution", "GUI Log", "認証", "GPU"], correctIndex: 0, explanation: "**_delta_log**: JSON Commit + 定期的 Checkpoint Parquet。**Snapshot Isolation / Optimistic Concurrency Control / Schema Evolution**。**Delta UniForm**(Iceberg / Hudi 互換、2024+)。" },
  { id: "dbx-q67", category: "Delta", difficulty: 3, question: "**`MERGE INTO`** の Delta Lake での用途として最も適切なものを選びなさい。", choices: ["Upsert + Delete + Insert を 1 SQL で実行(SCD Type 1/2 対応)", "INSERT のみ", "認証", "GPU"], correctIndex: 0, explanation: "**MERGE**: `WHEN MATCHED THEN UPDATE / DELETE` + `WHEN NOT MATCHED THEN INSERT`。**CDC / SCD Type 2 / Upsert** の中核。**Delta MERGE + Photon**で高速化。" },
  { id: "dbx-q68", category: "Photon", difficulty: 3, question: "**Photon Engine** の仕組みとして最も適切なものを選びなさい。", choices: ["C++ ベクトル化エンジン ・ JVM Spark の Op を高速化", "Python", "GUI", "認証"], correctIndex: 0, explanation: "**Photon**: Databricks 製 C++ Vectorized Engine。**Spark SQL / DataFrame の 2-3x 高速化**。**SQL Warehouse / DLT で標準有効**。**Pandas API on Spark** にも適用。" },
  { id: "dbx-q69", category: "DLT", difficulty: 3, question: "**Delta Live Tables の Expectations** の用途として最も適切なものを選びなさい。", choices: ["Data Quality Rule(`@dlt.expect`)→ 違反時 Warn / Fail / Drop", "学習用", "認証", "GPU"], correctIndex: 0, explanation: "**Expectations**: `@dlt.expect(\"valid_id\", \"id IS NOT NULL\")` / `expect_or_drop` / `expect_or_fail`。**品質メトリクス自動収集 → Event Log**。データ品質ゲートの第一線。" },
  { id: "dbx-q70", category: "Unity Catalog", difficulty: 3, question: "**Unity Catalog Volumes** の用途として最も適切なものを選びなさい。", choices: ["非構造化データ(画像 / PDF / モデル等)を Catalog で管理", "テーブル", "認証 SSO", "GPU"], correctIndex: 0, explanation: "**Volumes**(2023 GA): Catalog 内の **非テーブル**(画像 / PDF / モデル / Audio / Video)を ACL 統合 ・ Path アクセス。**Managed**(UC 管理)と **External**(既存パス参照)。" },
  { id: "dbx-q71", category: "Workflows", difficulty: 3, question: "**Databricks Workflows の Task Type** として **誤っているもの** を選びなさい。", choices: ["Notebook / Python Script / Python Wheel", "SQL / DLT / dbt / Spark Submit", "Run Job / If-Else / For Each", "GPU Task"], correctIndex: 3, explanation: "**Workflows Task**: Notebook / Python / Wheel / JAR / SQL / **DLT** / dbt / Spark Submit / Run Job / If-Else / For Each / Dashboard。**Job 間連携 + 条件分岐**で複雑 DAG 構築。" },
  { id: "dbx-q72", category: "Mosaic AI", difficulty: 3, question: "**Mosaic AI Vector Search** の特徴として最も適切なものを選びなさい。", choices: ["Delta Sync Index で Delta Table → 自動 Embedding 更新", "Standalone", "GUI", "認証"], correctIndex: 0, explanation: "**Vector Search**: **Delta Sync**(Auto Update)と **Direct Vector Access**(独立)。**HNSW + Filter + UC 統合**。**RAG / 推薦** で活用。" },
  { id: "dbx-q73", category: "Mosaic AI", difficulty: 3, question: "**Mosaic AI Model Serving** の Foundation Model API として **誤っているもの** を選びなさい。", choices: ["Pay-per-token(DBRX / Llama 3 / Mixtral 等)", "Provisioned Throughput(SLA 保証)", "External Model(OpenAI / Bedrock / Anthropic プロキシ)", "GPU 直販"], correctIndex: 3, explanation: "**Foundation Model API**: ① Pay-per-token(DBRX / Llama 3 / Mixtral / Mistral)② Provisioned Throughput ③ **External Model**(OpenAI / Bedrock / Anthropic / Cohere 統一プロキシ)。" },
  { id: "dbx-q74", category: "Mosaic AI", difficulty: 3, question: "**AI Functions(SQL)** の用途として最も適切なものを選びなさい。", choices: ["SQL から `ai_query / ai_summarize / ai_classify` 等で LLM 呼出", "ETL のみ", "GUI", "認証"], correctIndex: 0, explanation: "**AI Functions**: `ai_query / ai_extract / ai_classify / ai_summarize / ai_translate / ai_similarity / ai_mask / ai_gen / ai_forecast`。**SQL Warehouse から直接** LLM 利用 → アナリスト民主化。" },
  { id: "dbx-q75", category: "Genie", difficulty: 3, question: "**Databricks Genie**(AI/BI Genie)の用途として最も適切なものを選びなさい。", choices: ["NL2SQL: 自然言語 → Unity Catalog 横断質問応答", "ETL", "認証", "GPU"], correctIndex: 0, explanation: "**Genie**(2024 GA): NL → SQL → Visualization。**Unity Catalog Lineage + Sample Queries + Instructions** で精度向上。**AI/BI Dashboards** とも統合。" },
  { id: "dbx-q76", category: "DBRX", difficulty: 3, question: "**DBRX**(Databricks 製 OSS LLM)の特徴として最も適切なものを選びなさい。", choices: ["MoE 132B 総 / 36B Active ・ Apache 2.0 ・ Foundation Model API で提供", "100M Dense", "Closed", "GUI"], correctIndex: 0, explanation: "**DBRX**(2024 MosaicML / Databricks): 132B 総 / 36B Active MoE ・ コード / 数学に強い ・ Apache 2.0。**Mosaic AI Foundation Model API** で提供。Llama / Mistral 系と競合。" },
  { id: "dbx-q77", category: "Lakeflow", difficulty: 3, question: "**Lakeflow Connect** の用途として最も適切なものを選びなさい。", choices: ["SaaS / DB のマネージドコネクタ(Salesforce / Workday / SQL Server CDC 等)", "ETL コード手動", "認証", "GPU"], correctIndex: 0, explanation: "**Lakeflow Connect**(2024 GA): Fivetran ・ Stitch ・ Hevo 系統のマネージド取込。**Unity Catalog 直接取込**で**Bronze テーブル自動生成**。" },
  { id: "dbx-q78", category: "Security", difficulty: 3, question: "**Unity Catalog Row Filter / Column Mask** の用途として最も適切なものを選びなさい。", choices: ["細粒度アクセス制御(行 ・ 列単位で SQL 関数ベース)", "GUI のみ", "認証 SSO", "GPU"], correctIndex: 0, explanation: "**Row Filter**: `WHERE` 句注入。**Column Mask**: 値の変換(Hash / Redact)。**Dynamic Views**(2022)の後継 ・ **Tag-Based Access Control**(2024+)で大規模管理。" },
  { id: "dbx-q79", category: "Cost", difficulty: 3, question: "**Databricks DBU(Databricks Unit)** の特徴として **誤っているもの** を選びなさい。", choices: ["処理能力単位 ・ Cluster Type 別単価", "Workload Type(Jobs / All-Purpose / DLT / SQL)別単価", "Photon 有効で同 DBU でも高速", "GPU 直接課金"], correctIndex: 3, explanation: "**DBU**: Compute 単位。Tier(Standard / Premium / Enterprise)× Workload × Cluster Type。**Photon**(DBU 単価高だが高速)・ **Serverless**(管理楽だが DBU 増)で総コスト見極め。" },
  { id: "dbx-q80", category: "Cert", difficulty: 2, question: "**Databricks Data Engineer Associate** の Pre-requisite として最も適切なものを選びなさい。", choices: ["6 ヶ月以上の Databricks DE 経験推奨 ・ SQL / Spark 基礎必須", "未経験 OK", "ML Engineer 必須", "GPU 所有必須"], correctIndex: 0, explanation: "**DE Associate**: 6 ヶ月 + 経験推奨 ・ 45 問 ・ 90 分。**Delta Lake / DLT / Unity Catalog / Workflows / SQL Warehouse**。次は **DE Professional**(更に深い)。" },
];
