...
export const azureDp100Questions: Question[] = [
  ...
  { id: "dp100-q66", category: "AutoML", difficulty: 3, question: "**Azure ML AutoML** の入力 Task として **誤っているもの** を選びなさい。", choices: ["Classification / Regression", "Forecasting / Time Series", "Image / NLP Multi-class", "GPU Mining"], correctIndex: 3, explanation: "**AutoML Tasks**: Tabular(Classification / Regression / Forecasting)+ Vision(Image Classification / Object Detection)+ NLP(Text Class / NER)。**SDK + Studio Wizard** 両対応。" },
  { id: "dp100-q67", category: "Compute", difficulty: 3, question: "**Compute Cluster** vs **Compute Instance** の使い分けとして最も適切なものを選びなさい。", choices: ["Instance=個人開発 VM、Cluster=Auto Scale Job 実行(min/max ノード)", "両者同じ", "Instance=Cluster", "Cluster=単一 VM"], correctIndex: 0, explanation: "**Compute Instance**: 開発者個人の Jupyter / VS Code 用 VM。**Compute Cluster**: min/max 設定で Job 実行 ・ Auto Scale ・ Spot(Low-priority)対応。" },
  { id: "dp100-q68", category: "Data", difficulty: 3, question: "**Azure ML Datastore** の代表的バックエンドとして **誤っているもの** を選びなさい。", choices: ["Azure Blob / ADLS Gen2", "Azure Files / Data Lake Gen1", "Azure SQL DB / Snowflake", "GPU Storage"], correctIndex: 3, explanation: "**Datastore**: Blob / ADLS / Files / SQL / Snowflake 接続を抽象化。**MLTable / Data Asset** で バージョン管理 + Lineage。**Identity-based / Credential-based** 認証選択可。" },
  { id: "dp100-q69", category: "Sweep", difficulty: 3, question: "**Sweep Job** の **Early Termination Policy** として **誤っているもの** を選びなさい。", choices: ["Bandit Policy", "Median Stopping Policy", "Truncation Selection Policy", "GPU Termination"], correctIndex: 3, explanation: "**Early Termination**: Bandit(Slack from best)・ Median Stopping(median 以下)・ Truncation Selection(下位 X% 削除)・ No Termination。**Hyperdrive**(旧名)系。" },
  { id: "dp100-q70", category: "Pipeline", difficulty: 3, question: "**Component**(再利用可能 Pipeline 部品)の定義方法として **誤っているもの** を選びなさい。", choices: ["YAML(v2 SDK)", "@command_component デコレータ(Python)", "Designer(Drag&Drop)", "GPU Component"], correctIndex: 3, explanation: "**Component**: 入出力 + 環境 + コマンドを定義 → Workspace / Registry に登録 → 他 Pipeline で再利用。**YAML / Python / Designer GUI**で作成可。GPU Component は無関係。" },
  { id: "dp100-q71", category: "Endpoint", difficulty: 3, question: "**Managed Online Endpoint** の特徴として **誤っているもの** を選びなさい。", choices: ["低レイテンシ Real-time", "Blue/Green Deployment + Traffic Split", "Auto Scale + Authentication", "Batch 専用"], correctIndex: 3, explanation: "**Managed Online**: Real-time(<秒)・ Blue/Green + Traffic 設定 ・ Key/Token/AML Token 認証。**Batch Endpoint** は別途(大規模オフライン)。" },
  { id: "dp100-q72", category: "Endpoint", difficulty: 3, question: "**Batch Endpoint** の典型用途として最も適切なものを選びなさい。", choices: ["大量データのオフライン推論 ・ ParallelRunStep ベース", "Real-time", "学習用", "認証"], correctIndex: 0, explanation: "**Batch Endpoint**: Parallel Run + Input URI 指定 ・ 数億レコード非同期処理。**Pipeline Endpoint**(Pipeline 直公開)・ **Online Endpoint** と使い分け。" },
  { id: "dp100-q73", category: "Responsible AI", difficulty: 3, question: "**Responsible AI Dashboard** の機能として **誤っているもの** を選びなさい。", choices: ["Error Analysis(誤分類クラスタ)", "Interpretability(SHAP / Counterfactual)", "Causal Inference / Fairness", "GPU Tuning"], correctIndex: 3, explanation: "**RAI Dashboard**: Error Analysis + Model Interpretability(SHAP)+ Counterfactual + Causal + Fairness(Fairlearn)+ Data Explorer。**Generate Scorecard**(PDF)で経営報告。" },
  { id: "dp100-q74", category: "Distributed", difficulty: 3, question: "**Azure ML 分散学習** の Job 設定として **誤っているもの** を選びなさい。", choices: ["distribution.type: PyTorch / TensorFlow / MPI", "instance_count + process_count_per_instance", "DeepSpeed + Curated Environment 対応", "GPU Type 無関係"], correctIndex: 3, explanation: "**Distribution**: PyTorch(DDP)/ TensorFlow / MPI / Ray / Horovod。**Curated Environment**(DeepSpeed / FSDP / Megatron 等のプリインストール)で大規模学習簡素化。GPU Type は重要選択。" },
  { id: "dp100-q75", category: "Foundry", difficulty: 3, question: "**Azure AI Foundry**(旧 AI Studio)と **Azure ML** の関係として最も適切なものを選びなさい。", choices: ["Foundry が Generative AI 寄り ・ Azure ML が古典 ML / カスタム ・ 両者統合中", "完全別物", "Foundry が新規 ML", "Azure ML が廃止"], correctIndex: 0, explanation: "**Foundry vs Azure ML**: 2024 末から **Hub / Workspace 統合**進行。**Foundry**: Foundation Model + Agent + Prompt Flow + Eval。**Azure ML**: Classical ML / AutoML / MLOps / Custom Training。" },
  { id: "dp100-q76", category: "Fine-tuning", difficulty: 3, question: "**Azure ML / Foundry** で対応する Fine-tuning 方式として **誤っているもの** を選びなさい。", choices: ["Supervised Fine-Tuning(SFT)", "DPO / RFT(o-series)", "LoRA / QLoRA", "全モデル Full Pretraining"], correctIndex: 3, explanation: "**Fine-tuning**: GPT-3.5 / 4o-mini / Llama / Phi に対し **SFT / DPO / RFT / LoRA**(マネージド)対応。**Full Pretraining は基本提供されない**(コスト ・ 専門性で別契約)。" },
  { id: "dp100-q77", category: "Monitoring", difficulty: 3, question: "**Azure ML Model Monitor** の Drift 検知指標として **誤っているもの** を選びなさい。", choices: ["Wasserstein Distance", "Population Stability Index(PSI)", "Jensen-Shannon Divergence", "GPU Distance"], correctIndex: 3, explanation: "**Model Monitor**: Wasserstein / PSI / JS / Two-Sample KS / Normalized Wasserstein / Pearson's Chi-Square。**Data Drift / Prediction Drift / Feature Attribution Drift** を Schedule で監視。" },
  { id: "dp100-q78", category: "Security", difficulty: 3, question: "**Azure ML Private Workspace** の Network 構成として最も適切なものを選びなさい。", choices: ["Private Endpoint + Public Access Disabled + VNet Integration", "Public Endpoint のみ", "VPN なし", "GUI 認証のみ"], correctIndex: 0, explanation: "**Private Workspace**: Private Endpoint(Workspace / Storage / KV / ACR / AppInsights)+ Public Access OFF + VNet + Bastion / Jump Box。**金融 / 医療 / 政府** の規制対応に必須。" },
  { id: "dp100-q79", category: "Cost", difficulty: 3, question: "**Azure ML コスト最適化** として **誤っているもの** を選びなさい。", choices: ["Spot(Low-priority)VM の活用", "Compute Cluster min=0(idle 時 0 ノード)", "Reserved Instance / Savings Plan", "全 Cluster Max ノード常時起動"], correctIndex: 3, explanation: "**Cost 最適化**: Spot / Idle Shutdown / min=0 Cluster / Compute Instance Auto Shutdown / Reserved Capacity / Serverless Inference。**Always-on Max ノード**はコスト爆発。" },
  { id: "dp100-q80", category: "Cert", difficulty: 2, question: "**DP-100** の正式名称として最も適切なものを選びなさい。", choices: ["Designing and Implementing a Data Science Solution on Azure", "Solutions Architect Expert", "Azure AI Engineer", "Power BI Data Analyst"], correctIndex: 0, explanation: "**DP-100**: **Microsoft Certified: Azure Data Scientist Associate**。**実務 DS / ML Engineer**(Azure)向け Associate レベル。**AI-102(AI Engineer)/ DP-203(Data Engineer)** と並列。" },
];
