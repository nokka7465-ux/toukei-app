...
export const azureDp203Questions: Question[] = [
  ...
  { id: "dp203-q51", category: "Fabric", difficulty: 3, question: "**Microsoft Fabric** の **OneLake** の特徴として最も適切なものを選びなさい。", choices: ["テナント全体の単一 Data Lake(ADLS Gen2 互換)・ Shortcut で多 Workspace 共有", "Workspace 別に別 Lake", "GPU 専用", "暗号化なし"], correctIndex: 0, explanation: "**OneLake**: テナント当たり 1 つの **論理 Data Lake**。**Shortcut**(他 ADLS / S3 / GCS / Workspace を物理コピーなしで参照)・ **Delta-Parquet** ネイティブ。**Mirroring** で外部 DB を自動同期。" },
  { id: "dp203-q52", category: "Fabric", difficulty: 3, question: "Fabric **Lakehouse** と **Warehouse** の違いとして最も適切なものを選びなさい。", choices: ["Lakehouse=Spark / Notebook 中心 ・ 非構造化対応、Warehouse=T-SQL ・ 完全 ACID トランザクション", "両者同じ", "GPU vs CPU", "認証 vs 認可"], correctIndex: 0, explanation: "**Lakehouse**: Spark / Notebook / Delta / 半構造化対応。**Warehouse**: T-SQL 完全互換 ・ Multi-Table Transaction ・ ストアドプロシージャ。**両方 OneLake 上の Delta**で SQL Endpoint 経由相互参照可。" },
  { id: "dp203-q53", category: "Synapse", difficulty: 3, question: "Synapse **Serverless SQL Pool** の特徴として最も適切なものを選びなさい。", choices: ["ADLS 上の Parquet / CSV / Delta を OPENROWSET で直接クエリ ・ 従量課金", "GPU 必須", "Dedicated 専用", "GUI 専用"], correctIndex: 0, explanation: "**Serverless SQL Pool**: T-SQL で ADLS / Delta Lake を直接クエリ ・ **Data scanned 単位課金($5/TB)**。**Logical Data Warehouse(View / EXTERNAL TABLE)** を組める。" },
  { id: "dp203-q54", category: "Synapse", difficulty: 3, question: "Synapse **Dedicated SQL Pool** の **Distribution** で **Hash** を選ぶ典型シーンを選びなさい。", choices: ["大規模 Fact テーブル(>2GB)で JOIN キーが安定 → 並列性向上", "全テーブル", "Dim 専用", "ログ専用"], correctIndex: 0, explanation: "**Distribution**: **Hash**(高 Cardinality 列で並列化、Fact 向き)・ **Round-Robin**(キーなし、Stage 向き)・ **Replicate**(<2GB Dim、JOIN 移動なし)。MPP の中核。" },
  { id: "dp203-q55", category: "Synapse", difficulty: 3, question: "Synapse の **Partitioning** と **Clustered Columnstore Index(CCI)** の使い分けを選びなさい。", choices: ["Partition=日付など範囲(管理 ・ Switch In/Out)、CCI=列指向圧縮(>1M 行で圧縮 ・ Scan 高速)", "両者同じ", "OLTP vs OLAP", "GPU vs CPU"], correctIndex: 0, explanation: "**Partition**: 範囲分割(日 / 月)。**CCI(列ストア)**: 列圧縮 + 列単位 Scan ・ 1M 行 / Partition で十分。**Heap / CI(行ストア)** は Stage / Lookup 向き。" },
  { id: "dp203-q56", category: "ADF", difficulty: 3, question: "**Azure Data Factory / Synapse Pipeline** の **Integration Runtime(IR)** の種別として **誤っているもの** を選びなさい。", choices: ["Azure IR", "Self-Hosted IR(オンプレ接続)", "Azure SSIS IR", "GPU IR"], correctIndex: 3, explanation: "**IR 3 種**: **Azure IR**(クラウドネイティブ)・ **Self-Hosted IR**(オンプレ ・ VNet)・ **SSIS IR**(SSIS パッケージ実行 Lift-and-Shift)。GPU IR は存在しない。" },
  { id: "dp203-q57", category: "ADF", difficulty: 3, question: "ADF **Mapping Data Flow** の実行エンジンとして最も適切なものを選びなさい。", choices: ["Spark クラスター(自動 Scale-up / Scale-down)で GUI ベースの ETL を実行", "T-SQL", "Lambda", "GPU"], correctIndex: 0, explanation: "**Mapping Data Flow**: GUI で ETL 設計 → 内部で **Spark on Databricks 風クラスター**(自動起動 / シャットダウン)。**Wrangling Data Flow** は Power Query M ベース。" },
  { id: "dp203-q58", category: "Streaming", difficulty: 3, question: "**Azure Event Hubs** と **Service Bus** の使い分けとして最も適切なものを選びなさい。", choices: ["Event Hubs=高スループット Telemetry(Pub/Sub)、Service Bus=企業統合(キュー ・ トピック ・ 順序保証)", "両者同じ", "GPU vs CPU", "学習 vs 推論"], correctIndex: 0, explanation: "**Event Hubs**: 大規模 Telemetry / Log Streaming(Kafka 互換)・ Consumer Group。**Service Bus**: エンタープライズメッセージング(FIFO / Dead Letter / Transactions)。" },
  { id: "dp203-q59", category: "Streaming", difficulty: 3, question: "**Azure Stream Analytics** の **Windowing** で **Tumbling** vs **Hopping** の違いを選びなさい。", choices: ["Tumbling=固定 ・ 重複なし、Hopping=固定 + Hop で重複あり(Moving Avg 等)", "両者同じ", "GPU vs CPU", "学習 vs 推論"], correctIndex: 0, explanation: "**Tumbling**: 連続 ・ 重複なし固定窓(5 分集計)。**Hopping**: Hop(例: 1 分)で重複あり ・ 移動平均向き。**Sliding**: イベント発生時のみ評価。**Session**: 無活動時間で区切り。" },
  { id: "dp203-q60", category: "Storage", difficulty: 3, question: "**ADLS Gen2** の **Hierarchical Namespace(HNS)** の利点として最も適切なものを選びなさい。", choices: ["フォルダ Rename / Delete が O(1)・ POSIX 風 ACL ・ Big Data クエリ最適化", "GPU 高速化", "Endpoint", "認証 SSO"], correctIndex: 0, explanation: "**HNS**: 真の階層 FS(Blob のフラット構造でなく)。**Rename = メタデータ更新のみ(O(1))**・ POSIX ACL ・ Hadoop ABFS Driver / Delta 最適化。**有効化は作成時のみ**。" },
  { id: "dp203-q61", category: "Modeling", difficulty: 3, question: "**Slowly Changing Dimension(SCD)Type 2** の特徴として最も適切なものを選びなさい。", choices: ["変更履歴を新行 + EffectiveFrom / EffectiveTo / IsCurrent で保持", "上書きのみ(履歴失う)", "全部新テーブル", "削除のみ"], correctIndex: 0, explanation: "**SCD**: Type 1(上書き ・ 履歴なし)・ **Type 2**(履歴行追加 + 期間 + Flag)・ Type 3(列追加 ・ 直近 1 つ)・ Type 4(履歴別テーブル)。**Type 2 が DWH 標準**。" },
  { id: "dp203-q62", category: "Power BI", difficulty: 3, question: "**Power BI Direct Lake** モードの利点として最も適切なものを選びなさい。", choices: ["OneLake 上の Delta を Import なしで直接 Scan ・ Vertipaq の速度", "Import 最速", "DirectQuery 強化", "GPU 利用"], correctIndex: 0, explanation: "**Direct Lake**(Fabric Semantic Model): Delta-Parquet を OneLake から直接読み(コピーなし)・ Vertipaq 速度。**Import より新鮮 ・ DirectQuery より高速**。Fabric 最大の差別化。" },
  { id: "dp203-q63", category: "Purview", difficulty: 3, question: "**Microsoft Purview**(旧 Azure Purview)の用途として最も適切なものを選びなさい。", choices: ["データカタログ + 系譜(Lineage)+ 機密データ分類 + ガバナンスの統合", "学習用 GPU", "Endpoint", "認証 SSO"], correctIndex: 0, explanation: "**Purview**: マルチクラウド(Azure / AWS / GCP)+ オンプレを横断スキャン → カタログ ・ 系譜 ・ 機密データ分類(GDPR / PII)。**Data Map / Data Estate Insights**。" },
  { id: "dp203-q64", category: "Cosmos DB", difficulty: 3, question: "**Cosmos DB Synapse Link** の用途として最も適切なものを選びなさい。", choices: ["HTAP: 運用 DB(Cosmos)→ 分析 Store(列指向)を ETL なしで Synapse から SQL クエリ", "OLTP 高速化", "GPU", "認証"], correctIndex: 0, explanation: "**Synapse Link for Cosmos DB**: HTAP(Hybrid Transactional/Analytical Processing)。**Analytical Store(列指向 Auto-sync)**を Synapse Serverless / Spark から SQL/Spark クエリ。ETL 不要。" },
  { id: "dp203-q65", category: "実務", difficulty: 3, question: "DP-203 で頻出の **典型アーキテクチャ** として最も適切なものを選びなさい。", choices: ["Source → Event Hubs / ADF → ADLS Gen2 → Synapse Spark / SQL → Power BI(Medallion)", "EC2 のみ", "Lambda のみ", "RDS のみ"], correctIndex: 0, explanation: "**Medallion Architecture**: Bronze(Raw)→ Silver(Clean)→ Gold(Business)。**ADF / Event Hubs → ADLS → Synapse / Fabric → Power BI** が DP-203 / DP-600 共通の中核パターン。" },
  { id: "dp203-q66", category: "Fabric", difficulty: 3, question: "**Microsoft Fabric Capacity** の SKU として **誤っているもの** を選びなさい。", choices: ["F2 / F4 / F8 / ... / F2048", "Trial(60 days)", "Reserved 1y / 3y", "GPU SKU"], correctIndex: 3, explanation: "**Fabric Capacity**: F2 から F2048 までの CU(Capacity Unit)単位 ・ **Pay-as-you-go / Reserved**・ Trial(2024 廃止予定)・ **F64+ で Power BI Pro ライセンス代替**。GPU SKU は存在しない。" },
  { id: "dp203-q67", category: "Fabric", difficulty: 3, question: "**OneLake Shortcut** の対応 Source として **誤っているもの** を選びなさい。", choices: ["Amazon S3 / Azure ADLS", "Dataverse / Workspace 内", "GCP GCS / Iceberg(2024+)", "GPU メモリ"], correctIndex: 3, explanation: "**OneLake Shortcut**: 物理コピーなしの参照。**ADLS Gen2 / S3 / GCS / Dataverse / Iceberg(2024+)**。**Internal Shortcut**(他 Workspace)・ **Mirror**(SQL DB / Cosmos / Snowflake)も。" },
  { id: "dp203-q68", category: "Synapse", difficulty: 3, question: "**Synapse Dedicated SQL Pool** の Distribution として **誤っているもの** を選びなさい。", choices: ["Hash(高 Cardinality 列)", "Round-Robin(キーなし、Stage)", "Replicate(<2 GB Dim)", "GPU Distribution"], correctIndex: 3, explanation: "**Distribution**: Hash / Round-Robin / Replicate。**MPP**(Massively Parallel Processing)の中核 ・ Fact = Hash / Stage = RR / Dim = Replicate が定番。" },
  { id: "dp203-q69", category: "Synapse", difficulty: 3, question: "**Synapse Serverless SQL Pool** の課金単位として最も適切なものを選びなさい。", choices: ["スキャンしたデータ量($5/TB)", "クエリ回数", "GPU 時間", "認証回数"], correctIndex: 0, explanation: "**Serverless SQL**: Athena と同様のスキャン課金。**OPENROWSET**で ADLS の Parquet / CSV / Delta 直接クエリ。**Logical Data Warehouse**を View で構築可。" },
  { id: "dp203-q70", category: "ADF", difficulty: 3, question: "**Mapping Data Flow** の実行エンジンとして最も適切なものを選びなさい。", choices: ["Spark Cluster(自動起動 / シャットダウン)", "T-SQL", "Lambda", "GPU 専用"], correctIndex: 0, explanation: "**Mapping Data Flow**: GUI で ETL 設計 → **Databricks ベースの Spark Cluster** が裏側。**Wrangling Data Flow**(Power Query M)とは別物。" },
  { id: "dp203-q71", category: "Streaming", difficulty: 3, question: "**Event Hubs Capture** の用途として最も適切なものを選びなさい。", choices: ["Event を自動で ADLS / Blob に Parquet/Avro 保存", "認証", "GPU 監視", "OLTP"], correctIndex: 0, explanation: "**Event Hubs Capture**: ストリーミングデータを **追加コードなしで** ADLS / Blob にバッチ保存 → DWH / Synapse での分析へ。**Real-time + Batch のハイブリッド構成**。" },
  { id: "dp203-q72", category: "Stream Analytics", difficulty: 3, question: "**Azure Stream Analytics** の Windowing として **誤っているもの** を選びなさい。", choices: ["Tumbling(固定 ・ 重複なし)", "Hopping(固定 + Hop で重複あり)", "Sliding(イベントごと)/ Session(無活動区切り)", "GPU Window"], correctIndex: 3, explanation: "**ASA Windowing**: Tumbling / Hopping / Sliding / Session / Snapshot。**Spark Structured Streaming / Beam(Dataflow)** の Window と同様コンセプト。" },
  { id: "dp203-q73", category: "Storage", difficulty: 3, question: "**ADLS Gen2 Hierarchical Namespace(HNS)** の利点として最も適切なものを選びなさい。", choices: ["真の階層 FS ・ Rename O(1) ・ POSIX ACL ・ ABFS Driver 最適化", "Flat Blob のみ", "GPU 高速", "認証 SSO"], correctIndex: 0, explanation: "**HNS**: Blob のフラット構造 vs **真の階層 FS**。**Rename = メタ更新 O(1)**・ POSIX ACL ・ Hadoop ABFS Driver / Delta Lake 最適化。**作成時のみ有効化可**。" },
  { id: "dp203-q74", category: "Cosmos DB", difficulty: 3, question: "**Cosmos DB Synapse Link** の特徴として最も適切なものを選びなさい。", choices: ["HTAP: Analytical Store(列指向)を Synapse から直接 SQL/Spark で分析", "OLTP 高速化", "GPU", "認証"], correctIndex: 0, explanation: "**Cosmos DB Synapse Link**: HTAP 実装。Tx Store(行指向)→ Analytical Store(列指向 Auto-sync)→ **Synapse Serverless / Spark で ETL なし分析**。" },
  { id: "dp203-q75", category: "Modeling", difficulty: 3, question: "**SCD Type 2(Slowly Changing Dimension)** の特徴として最も適切なものを選びなさい。", choices: ["変更を新行 + Valid From/To + IsCurrent で履歴保持", "上書きのみ", "新テーブル作成", "認証"], correctIndex: 0, explanation: "**SCD Type 2**: Dim の履歴保持 ・ 過去事実との整合性確保。**Type 1**(上書き)・ **Type 3**(列追加)・ **Type 4**(履歴別テーブル)。DWH 設計の必修。" },
  { id: "dp203-q76", category: "Direct Lake", difficulty: 3, question: "**Power BI Direct Lake モード**(Fabric 専用)の特徴として最も適切なものを選びなさい。", choices: ["OneLake Delta を Vertipaq に直接読込 → Import 並速度 + 鮮度", "DirectQuery のみ", "Import 強化", "GPU"], correctIndex: 0, explanation: "**Direct Lake**: Fabric Semantic Model 専用。**Delta-Parquet → メモリ Vertipaq**(コピーなし、On-Demand 読込)。**Hybrid Tables** で Import + DQ も組合せ可。" },
  { id: "dp203-q77", category: "Purview", difficulty: 3, question: "**Microsoft Purview** の用途として最も適切なものを選びなさい。", choices: ["データカタログ + Lineage + 機密データ分類 + ガバナンス統合", "OLTP", "GPU 監視", "認証"], correctIndex: 0, explanation: "**Purview**(2022 統合): **Microsoft Purview Data Map / Catalog / Insights**。**Azure / AWS / GCP / SaaS / オンプレ**を横断スキャン ・ GDPR / PII / 系譜追跡。" },
  { id: "dp203-q78", category: "Security", difficulty: 3, question: "**Synapse Always Encrypted** の特徴として最も適切なものを選びなさい。", choices: ["クライアント側で暗号化 → DB 管理者にも平文不可", "サーバ側のみ", "全テーブル一律", "GPU 暗号化"], correctIndex: 0, explanation: "**Always Encrypted**: 列単位 ・ クライアント暗号化。**Secure Enclaves** 版で範囲検索 / パターンマッチも可。**TDE**(透過)とは別レベルの保護。" },
  { id: "dp203-q79", category: "Cost", difficulty: 3, question: "**Synapse Workspace のコスト最適化** として **誤っているもの** を選びなさい。", choices: ["Dedicated SQL Pool Pause", "Serverless で ad-hoc + Spark Pool Auto Pause", "Reserved Capacity / Compute", "全 Pool 24/7 起動"], correctIndex: 3, explanation: "**Cost 最適化**: Pause / Serverless / Auto Pause / Reserved。**Always-on 全 Pool** はコスト爆発。**Cost Management**(Azure)で可視化 ・ Budget Alert 設定。" },
  { id: "dp203-q80", category: "Cert", difficulty: 2, question: "**DP-203** の正式名称として最も適切なものを選びなさい。", choices: ["Microsoft Certified: Azure Data Engineer Associate", "Solutions Architect Expert", "AI Engineer Associate", "Power BI Data Analyst"], correctIndex: 0, explanation: "**DP-203**: **Azure Data Engineer Associate**。**DP-600 / DP-700(Fabric Associate)** が後継的位置づけ。**実務 DE on Azure** の認定。" },
];
