...
export const pythonDataQuestions: Question[] = [
  ...
  { id: "pyd-q51", category: "NumPy", difficulty: 3, question: "**`np.where(condition, x, y)`** の挙動として正しいものを選びなさい。", choices: ["condition が True の位置に x、False の位置に y を返す", "x のみ返す", "ランダム選択", "DB クエリ"], correctIndex: 0, explanation: "**`np.where`**: ベクトル化された if-else。例: `np.where(arr > 0, arr, 0)` で負値を 0 に。**条件のみで True 位置のインデックスを返すモード**もあり。" },
  { id: "pyd-q52", category: "pandas", difficulty: 3, question: "pandas で **時系列インデックスの周波数変換** に使うメソッドを選びなさい。", choices: ["resample()", "groupby()", "pivot()", "melt()"], correctIndex: 0, explanation: "**`df.resample('M').mean()`**: 月次集計。**`asfreq()`** は時間軸変更のみ。Pandas の時系列処理の基本機能。" },
  { id: "pyd-q53", category: "pandas", difficulty: 3, question: "pandas の **`.loc[]` と `.iloc[]`** の違いとして正しいものを選びなさい。", choices: [".loc: ラベル指定 / .iloc: 整数位置", "両者同じ", ".loc は廃止", ".iloc は文字列のみ"], correctIndex: 0, explanation: "**`.loc[row_label, col_label]`** はラベルベース。**`.iloc[row_pos, col_pos]`** は整数位置。** `df.loc[df['x'] > 0, 'y']`** など条件付きアクセスで頻用。" },
  { id: "pyd-q54", category: "可視化", difficulty: 3, question: "**Matplotlib** で **散布図** を描く関数として最も適切なものを選びなさい。", choices: ["plt.scatter(x, y)", "plt.plot(x, y)", "plt.bar(x, y)", "plt.hist(x)"], correctIndex: 0, explanation: "**`plt.scatter`**: 散布図 + マーカーサイズ / 色を c, s で制御。**`plt.plot`** は線、**`plt.bar`** は棒、**`plt.hist`** はヒストグラム。" },
  { id: "pyd-q55", category: "scikit-learn", difficulty: 3, question: "**`StandardScaler`** の役割として正しいものを選びなさい。", choices: ["平均 0 / 分散 1 に標準化(Z スコア化)", "Min-Max 正規化", "対数変換", "PCA"], correctIndex: 0, explanation: "**StandardScaler**: $(x - \\mu) / \\sigma$。**MinMaxScaler** は $[0, 1]$ にスケール。NN / KNN / SVM / 主成分分析の前処理で必須。" },
  { id: "pyd-q56", category: "scikit-learn", difficulty: 3, question: "**`Pipeline`** の主な利点として最も適切なものを選びなさい。", choices: ["前処理 + モデルを 1 オブジェクト化 + CV で Leakage 回避", "速度", "GPU 利用", "GUI"], correctIndex: 0, explanation: "**Pipeline**: `[('scaler', StandardScaler()), ('clf', LogisticRegression())]` のように連結。**CV 内で各 Fold ごとに前処理を fit** → Data Leakage 防止。" },
  { id: "pyd-q57", category: "scikit-learn", difficulty: 3, question: "**`GridSearchCV`** と **`RandomizedSearchCV`** の使い分けとして正しいものを選びなさい。", choices: ["Grid: 全組合せ ・ 小空間、Random: ランダム ・ 大空間", "両者同じ", "Grid は廃止", "Random は遅い"], correctIndex: 0, explanation: "**GridSearchCV**: 全組合せ網羅 → 小ハイパー空間。**RandomizedSearchCV**: 指定数ランダム → 大空間 ・ 連続パラメータ。**BayesSearchCV / Optuna** は更に効率的。" },
  { id: "pyd-q58", category: "ML 評価", difficulty: 3, question: "**`classification_report`** が表示する指標として **誤っているもの** を選びなさい。", choices: ["Precision / Recall / F1", "Support(サンプル数)", "Macro / Weighted Average", "GPU 利用率"], correctIndex: 3, explanation: "**`classification_report`**: クラスごと P / R / F1 + Support + Macro / Weighted Avg。**`confusion_matrix`** は混同行列。GPU は無関係。" },
  { id: "pyd-q59", category: "Python", difficulty: 3, question: "Python の **list comprehension** の **正しい構文** を選びなさい。", choices: ["[expr for x in iterable if cond]", "for x in iterable: yield expr", "(expr for x in)", "map(lambda)"], correctIndex: 0, explanation: "**List comp**: `[x**2 for x in range(10) if x % 2 == 0]`。**Generator**: `(...)` で memory 効率良。**Dict / Set comp** も同様(`{}` 使用)。" },
  { id: "pyd-q60", category: "Python", difficulty: 3, question: "**`lambda` 式** の用途として最も適切なものを選びなさい。", choices: ["短い無名関数(map / filter / sort key)", "ループ", "クラス定義", "DB クエリ"], correctIndex: 0, explanation: "**lambda**: `lambda x: x**2`。**map / filter / sorted / pandas apply** で頻用。複雑になる場合は `def` 関数で書く。" },
  { id: "pyd-q61", category: "Visualization", difficulty: 3, question: "**Plotly** が **Matplotlib** より優れる点として最も適切なものを選びなさい。", choices: ["インタラクティブ(Zoom / Hover / 3D)", "速度", "GUI のみ", "GPU 利用"], correctIndex: 0, explanation: "**Plotly**: Web ベースのインタラクティブ可視化。**Dashboard(Dash)** で SPA 構築可。Matplotlib は静的画像出力に強い。" },
  { id: "pyd-q62", category: "ML 実装", difficulty: 3, question: "**imbalanced-learn** の **SMOTE** が **scikit-learn の `class_weight='balanced'`** と異なる点として最も適切なものを選びなさい。", choices: ["SMOTE: 少数クラスを合成生成 / class_weight: 損失重み調整", "両者同じ", "SMOTE は廃止", "class_weight は不正確"], correctIndex: 0, explanation: "**SMOTE**: k-NN ベースで少数クラスサンプル合成 → 学習データの実バランス変更。**class_weight**: 損失関数で重み付け(データ変更なし)。両者併用も可。" },
  { id: "pyd-q63", category: "DataFrame", difficulty: 3, question: "**Polars** の **Lazy API** の利点として最も適切なものを選びなさい。", choices: ["クエリ最適化 + 必要時のみ実行 → メモリ削減 + 高速化", "全カラム読込", "GPU 必須", "DB 接続"], correctIndex: 0, explanation: "**Polars Lazy**: `pl.scan_csv()` → 必要操作を組合せ → `.collect()` で最適化 + 実行。**Push-down(Predicate / Projection)** で I/O ・ 計算を削減。" },
  { id: "pyd-q64", category: "DuckDB", difficulty: 3, question: "**DuckDB** の **特徴** として最も適切なものを選びなさい。", choices: ["In-process OLAP DB + Pandas/Arrow ゼロコピー連携", "リモート DB", "GUI 専用", "Stream 専用"], correctIndex: 0, explanation: "**DuckDB**: SQLite 風のインプロセス + 列指向 + Pandas / Arrow 直接クエリ。**ローカル分析の新標準** で Notebook / ETL に最適。" },
  { id: "pyd-q65", category: "学習", difficulty: 1, question: "Python データ分析試験 → **より実践的な認定** として最も適切なものを選びなさい。", choices: ["Python 3 エンジニア認定データ分析実践試験", "ITパスポート", "簿記", "再受験"], correctIndex: 0, explanation: "**PythonED**: Python 3 エンジニア認定基礎 → データ分析 → データ分析実践 → 実践基礎 と段階的に。**G 検定 / DS 検定 / Kaggle Competition** で実務力も。" },
  { id: "pyd-q66", category: "NumPy", difficulty: 3, question: "**`np.broadcasting`** の動作として最も適切なものを選びなさい。", choices: ["異なる形状の配列を自動的に拡張して演算可能にする", "ループ展開", "GPU 演算", "認証"], correctIndex: 0, explanation: "**Broadcasting**: shape を後ろから比較 → 1 か同じならスケール。**例**: (3,1) + (1,4) → (3,4)。**メモリ拡張せず効率的**。Pandas / PyTorch / TF も同様の仕組み。" },
  { id: "pyd-q67", category: "NumPy", difficulty: 3, question: "**`np.einsum('ij,jk->ik', A, B)`** が実行する演算として最も適切なものを選びなさい。", choices: ["行列積 A @ B(`np.matmul(A, B)` と等価)", "要素積", "転置", "逆行列"], correctIndex: 0, explanation: "**einsum**: Einstein 記法。`ij,jk->ik` は j で縮約 → 行列積。**`ij->ji` で転置 / `ii->` でトレース / `ij,ij->` で内積**。PyTorch / TF / JAX でも同記法。" },
  { id: "pyd-q68", category: "pandas", difficulty: 3, question: "**`df.groupby('col').agg({'x': ['mean', 'sum']})`** の結果として最も適切なものを選びなさい。", choices: ["col 別に x の平均と合計を計算 → MultiIndex Column", "全体平均", "x のみ", "認証"], correctIndex: 0, explanation: "**groupby + agg**: 複数集計を一度に。**`as_index=False`** で MultiIndex 回避。**`pipe / transform / filter / apply`** も使い分ける。**`pd.NamedAgg`** で列名指定も可。" },
  { id: "pyd-q69", category: "pandas", difficulty: 3, question: "**`pd.merge(left, right, on='key', how='left')`** と **SQL LEFT JOIN** の関係として最も適切なものを選びなさい。", choices: ["完全に等価(キー一致は inner、左側のみ key は NULL 埋め)", "両者異なる", "Inner Join", "Full Outer Join"], correctIndex: 0, explanation: "**pd.merge**: SQL JOIN と等価。**how=left/right/inner/outer/cross**。**`indicator=True`** で source 列追加。**`validate='1:1'`** でカーディナリティ検証も。" },
  { id: "pyd-q70", category: "pandas", difficulty: 3, question: "**`df.pivot_table` と `df.pivot`** の違いとして最も適切なものを選びなさい。", choices: ["pivot_table=集計関数あり ・ 重複対応、pivot=純粋な形状変換 ・ 重複不可", "両者同じ", "pivot=集計", "pivot_table=形状のみ"], correctIndex: 0, explanation: "**pivot**: long → wide の変換のみ ・ 重複キーでエラー。**pivot_table**: aggfunc 指定可 ・ NaN 補完 ・ margins(合計列)対応。Excel ピボットテーブル相当。" },
  { id: "pyd-q71", category: "可視化", difficulty: 3, question: "**`matplotlib`** vs **`seaborn`** の使い分けとして最も適切なものを選びなさい。", choices: ["matplotlib=低レベル細かい制御、seaborn=統計プロット高速作成(matplotlib ラッパ)", "両者完全独立", "seaborn=GUI", "matplotlib=Web"], correctIndex: 0, explanation: "**matplotlib**: フル制御 ・ Figure / Axes 操作。**seaborn**: 統計プロット(violinplot / boxplot / heatmap / pairplot)・ DataFrame と親和。**Plotly**(インタラクティブ)・ **Altair**(宣言的)も選択肢。" },
  { id: "pyd-q72", category: "scikit-learn", difficulty: 3, question: "**`sklearn.pipeline.Pipeline`** の利点として最も適切なものを選びなさい。", choices: ["前処理 + モデルを連結 → Data Leakage 防止 + Grid Search 統合", "GPU 高速化", "認証", "DB 接続"], correctIndex: 0, explanation: "**Pipeline**: `[('scaler', StandardScaler()), ('clf', LogReg())]`。**fit/transform 自動連鎖**。**ColumnTransformer**(列別前処理)・ **FeatureUnion**(並列特徴)も。**CV 内 fit** で Leakage 防止。" },
  { id: "pyd-q73", category: "scikit-learn", difficulty: 3, question: "**`GridSearchCV` vs `RandomizedSearchCV`** の使い分けとして最も適切なものを選びなさい。", choices: ["Grid=全組合せ網羅 ・ 小規模、Randomized=ランダムサンプリング ・ 大規模 / 連続値", "両者同じ", "Grid=高速", "Randomized=低精度"], correctIndex: 0, explanation: "**Grid**: 全組合せ。**Randomized**: 指定回数ランダム選択 ・ 大規模 / 連続パラメータに有効。**BayesSearchCV**(scikit-optimize)・ **Optuna**(TPE)で更に効率化。" },
  { id: "pyd-q74", category: "scikit-learn", difficulty: 3, question: "**`StandardScaler` vs `MinMaxScaler`** の使い分けとして最も適切なものを選びなさい。", choices: ["Standard=平均 0 / 分散 1 ・ 外れ値耐性低、MinMax=[0,1]・ NN / 画像で多用", "両者同じ", "MinMax=分散 1", "Standard=範囲 [0,1]"], correctIndex: 0, explanation: "**StandardScaler**: $(x - \\mu) / \\sigma$。**MinMaxScaler**: $(x - \\min) / (\\max - \\min)$。**RobustScaler**(外れ値耐性、IQR ベース)・ **PowerTransformer**(歪み補正)も。" },
  { id: "pyd-q75", category: "実装", difficulty: 3, question: "**`sklearn` で Imbalanced Classification** の対策として **誤っているもの** を選びなさい。", choices: ["`class_weight='balanced'`", "SMOTE(imbalanced-learn)", "Threshold Tuning", "全データ削除"], correctIndex: 3, explanation: "**Imbalanced 対策**: class_weight / Oversampling(SMOTE / ADASYN)/ Undersampling / Threshold 調整 / F1 / PR AUC 評価 / Cost-sensitive Learning。**全削除は本末転倒**。" },
  { id: "pyd-q76", category: "GBDT", difficulty: 3, question: "**LightGBM / XGBoost / CatBoost** の特徴比較として **誤っているもの** を選びなさい。", choices: ["LightGBM: Leaf-wise + 高速", "XGBoost: 業界標準 + 柔軟", "CatBoost: カテゴリ変数自動処理", "全て GPU 必須"], correctIndex: 3, explanation: "**LightGBM**: Microsoft / Leaf-wise / Histogram / 高速。**XGBoost**: 業界標準 / Level-wise / 柔軟。**CatBoost**: Yandex / カテゴリ Encoding 自動 / 過学習対策。**全て CPU でも動作**(GPU は加速版)。" },
  { id: "pyd-q77", category: "EDA", difficulty: 3, question: "**`pandas-profiling / ydata-profiling`** の用途として最も適切なものを選びなさい。", choices: ["DataFrame の自動 EDA レポート生成(統計 / 相関 / 欠損 / 重複)", "GUI BI", "GPU 監視", "認証"], correctIndex: 0, explanation: "**ydata-profiling**(旧 pandas-profiling): 1 行で EDA HTML レポート。**Sweetviz / D-Tale / AutoViz / Lux** も同種ツール。初期 EDA 高速化に有効。" },
  { id: "pyd-q78", category: "高速化", difficulty: 3, question: "**Polars** と **pandas** の関係として最も適切なものを選びなさい。", choices: ["Polars=Rust 実装 ・ Lazy Evaluation で大規模高速、pandas=Eager ・ 業界標準", "両者完全同じ", "Polars=GUI", "pandas=GPU"], correctIndex: 0, explanation: "**Polars**: Rust / Apache Arrow ベース ・ 並列処理 ・ Lazy(クエリ最適化)・ pandas 比 5-10x 高速。**DuckDB**(SQL ・ OLAP)も同種。**Modin / cuDF**(GPU)も大規模 pandas 互換。" },
  { id: "pyd-q79", category: "MLOps", difficulty: 3, question: "**Python 環境管理** の代表手法として **誤っているもの** を選びなさい。", choices: ["venv / pip", "Poetry / pip-tools(lock file)", "uv(2024+ 超高速 Rust)/ Conda", "Microsoft Word"], correctIndex: 3, explanation: "**Python 環境**: venv(標準)・ Poetry / Pipenv / PDM(lock + dep)・ Conda(科学計算)・ **uv**(2024+ Astral 製 ・ Rust ・ 超高速)。**requirements.txt / pyproject.toml** で依存記述。" },
  { id: "pyd-q80", category: "実務", difficulty: 1, question: "Python データ分析試験で重視される **総合データ分析力** として最も適切なものを選びなさい。", choices: ["NumPy + pandas + matplotlib + scikit-learn + 統計基礎 + 業務適用判断の統合", "Python 文法のみ", "GUI 操作のみ", "数学のみ"], correctIndex: 0, explanation: "**PythonED 範囲**: ① Python 基礎 ② NumPy ・ pandas ・ matplotlib ・ scikit-learn ・ scipy ③ 統計 / ML 基礎 ④ Jupyter 環境。**実務データ分析の事実上標準**。" },
];
